...
const textExtensions = new Set([".ts", ".tsx", ".js", ".mjs", ".json", ".css", ".svg", ".webmanifest"]);
...
const files = sourceRoots.flatMap((folder) => walk(join(root, folder))).filter((path) => textExtensions.has(extension(path)));
const violations = [];
const forbidden = [
  { label: "simulated data marker", pattern: /\b(?:mock|fake)[-_ ]?(?:api|data|token|user|response)?\b/i },
  { label: "demonstration account marker", pattern: /\bdemo[-_ ]?(?:account|user|token|data)\b/i },
...
for (const path of requiredFiles) {
  try {
    if (!statSync(join(root, path)).isFile()) violations.push(`${path}: required file is missing`);
  } catch {
...
const messageFiles = readdirSync(messageDirectory).filter((name) => name.endsWith(".json")).sort();
const expectedLocales = ["ar.json", "de.json", "en.json", "es.json", "fr.json", "tr.json"];
...
for (const name of messageFiles) {
  ...
  if (JSON.stringify(keys) !== JSON.stringify(referenceKeys)) {
    violations.push(`src/messages/${name}: translation keys do not match en.json`);
  }
}
...
for (const route of [
  ...
  if (!apiSource.includes(route)) violations.push(`src/lib/api.ts: confirmed route missing: ${route}`);
}
...
console.log(`Integrity check passed: ${files.length} files, ${messageFiles.length} complete locales, no simulated data markers.`);
