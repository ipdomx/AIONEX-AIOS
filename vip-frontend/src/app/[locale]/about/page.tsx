...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return localizedMetadata({ locale, path: "about", title: t("aboutTitle"), description: t("aboutDescription") });
}
      ...
      <section className="section-pad">
        <div className="page-shell grid gap-6 lg:grid-cols-2">
          <article className="glass-panel rounded-3xl p-8 sm:p-10">
            <Target className="h-8 w-8 text-electric-200" aria-hidden="true" />
            <h2 className="mt-6 text-2xl font-semibold">{t("missionTitle")}</h2>
            <p className="mt-4 text-base leading-8 text-white/55">{t("missionCopy")}</p>
          </article>
          <article className="glass-panel rounded-3xl p-8 sm:p-10">
            <Eye className="h-8 w-8 text-violet-400" aria-hidden="true" />
            <h2 className="mt-6 text-2xl font-semibold">{t("visionTitle")}</h2>
            <p className="mt-4 text-base leading-8 text-white/55">{t("visionCopy")}</p>
          </article>
        </div>
      </section>
        ...
        <div className="page-shell">
          ...
          <div className="mt-12 grid gap-4 sm:grid-cols-2">
            {principles.map(({ icon: Icon, title, copy }) => (
              <article key={title} className="rounded-2xl border border-white/[0.07] bg-white/[0.025] p-6 sm:p-8">
                <Icon className="h-6 w-6 text-electric-200" aria-hidden="true" />
