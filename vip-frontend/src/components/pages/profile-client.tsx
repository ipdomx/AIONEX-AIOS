...
import { AccountSecurityManager } from "@/components/auth/account-security-manager";
  ...
  async function saveProfile(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const result = await updateSettings({
        avatar: avatar || "",
        ...(!isFree && name.trim() !== settings.profile.name
          ? { name: name.trim() }
          : {}),
      });
      setSettings(result);
          ...
          >
            ...
            <div className="mt-5">
              ...
              <input
                ...
                type="password"
                className="field-control"
                minLength={settings.security.password_min_length}
                maxLength={256}
              ...
              <p className="mt-2 text-xs text-white/35">
                {t("passwordMinimum", {
                  count: settings.security.password_min_length,
                })}
              </p>
            </div>
            <div className="mt-5">
            ...
            <div className="mt-6 flex items-start gap-3 rounded-xl border border-electric-300/15 bg-electric-400/[0.06] p-4 text-xs leading-6 text-white/45">
              <ShieldCheck
                className="mt-0.5 h-4 w-4 shrink-0 text-electric-200"
                aria-hidden="true"
              />
