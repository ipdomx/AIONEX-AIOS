...
function inferCountryCode(): string {
  ...
  for (const value of [...navigator.languages, navigator.language]) {
    ...
    try {
      ...
      // Fall back to a syntactically valid region subtag when Intl rejects it.
    }
...
export function RegisterClient() {
  ...
  const [phoneConfig, setPhoneConfig] =
  ...
  const [challenge, setChallenge] = useState<FirebasePhoneChallenge | null>(
    null,
  );
  ...
  const maxBirthDate = useMemo(() => {
    const date = new Date();
    date.setUTCFullYear(
      date.getUTCFullYear() - (policy?.identity.minimum_age || 18),
    );
  ...
  useEffect(() => {
    setCountry((current) => current || inferCountryCode());
  }, []);
  ...
  async function sendPhoneCode() {
    ...
    try {
      const readiness = await getFirebasePhoneReadiness(
        phone,
        window.location.origin,
      );
      if (!readiness.ready)
        throw new Error(readiness.detail || t("phoneUnavailable"));
      disposeFirebasePhoneChallenge(challenge);
    ...
    return (
      <div className="page-shell section-pad max-w-2xl">
        <StatusMessage tone="error">
          {configError || t("configurationError")}
        </StatusMessage>
        <Button
          variant="secondary"
          className="mt-5"
  ...
  if (!policy.enabled) {
    return (
      <div className="page-shell section-pad max-w-2xl">
        <StatusMessage>{t("registrationClosed")}</StatusMessage>
      </div>
    );
  }
          ...
          <div className="glass-panel mt-8 rounded-2xl p-5">
            ...
            <dl className="mt-4 space-y-3 text-sm text-white/50">
              <div className="flex justify-between gap-4">
                <dt>{t("projectsLimit")}</dt>
                <dd className="font-semibold text-white">
                  {policy.limits.projects}
                </dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt>{t("messagesLimit")}</dt>
                <dd className="font-semibold text-white">
                  {policy.limits.user_messages_per_month}
                </dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt>{t("minimumAge")}</dt>
                <dd className="font-semibold text-white">
                  {policy.identity.minimum_age}
                </dd>
              </div>
            </dl>
            <p className="mt-4 border-t border-white/[0.07] pt-4 text-xs leading-6 text-white/35">
              {t("ownerControlled")}
            </p>
          </div>
        </aside>
        ...
        <div className="glass-panel rounded-3xl p-6 sm:p-9">
          ...
          <form onSubmit={submit}>
            <div className="grid gap-5 sm:grid-cols-2">
              ...
              <div>
                <label htmlFor="register-birth" className="field-label">
                  {t("birthDate")}
                </label>
                <input
                  id="register-birth"
              ...
              <div>
                <label htmlFor="register-country" className="field-label">
                  {t("countryCode")}
                </label>
                <input
                  id="register-country"
            ...
            <div className="mt-7 rounded-2xl border border-white/[0.08] bg-black/15 p-5 sm:p-6">
              ...
              {(phoneStage === "code" || phoneStage === "confirming") && (
                <div className="mt-4 flex flex-col gap-3 sm:flex-row">
                  <input
                    inputMode="numeric"
                    className="field-control flex-1 tracking-[0.3em]"
                  ...
                  <Button
                    ...
                    disabled={
                      phoneStage === "confirming" || verificationCode.length < 6
                    }
                  >
                    {phoneStage === "confirming" ? (
                      <LoaderCircle className="h-4 w-4 animate-spin" />
            ...
            <label className="mt-6 flex cursor-pointer items-start gap-3 rounded-xl border border-white/[0.07] p-4 text-sm leading-6 text-white/50">
              ...
              <span>
                ...
                <Link
                  ...
                  className="text-electric-200 hover:underline"
                >
                  {t("terms")}
                </Link>{" "}
                {t("and")}{" "}
            ...
            <Button
              ...
              disabled={
                submitting ||
                (phoneRequired && (!phoneReady || !firebaseIdToken))
              }
            >
              {submitting ? (
                <LoaderCircle className="h-4 w-4 animate-spin" />
