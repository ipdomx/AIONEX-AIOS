...
ROOT=Path('/opt/AIOS'); IMAGE=Path('/var/lib/aionex/fr06-vaults/container-runtime-vault.luks2'); MAPPER_NAME='aionex-container-runtime-vault'; MAPPER=Path('/dev/mapper')/MAPPER_NAME; MOUNT=Path('/mnt/aionex/fr06-container-runtime-vault'); STATE=Path('/var/lib/aionex/fr06c4-runtime'); RUNTIME=Path('/run/aionex-fr06c4-runtime'); KEYS=Path('/dev/shm/aionex-fr06c4-runtime-keys'); SIZE=64*1024**3; OPTS=('nodev','nosuid'); CONFIRM='PROVISION_FR06C4_EMPTY_RUNTIME_VAULT'; UNLOCK_CONFIRM='UNLOCK_FR06C4_RUNTIME_VAULT'; MAX_TTL=900
...
def fsha(p):
 ...
 return h.hexdigest()
def private(p,label,maxsize=65536):
 try:s=os.lstat(p)
 except OSError as x:raise B(f'{label} unavailable') from x
 if stat.S_ISLNK(s.st_mode) or not stat.S_ISREG(s.st_mode) or s.st_nlink!=1 or s.st_uid!=0 or stat.S_IMODE(s.st_mode)&0o077 or not 1<=s.st_size<=maxsize:raise B(f'{label} unsafe')
def bundle(p,purpose):
 ...
 if set(d)!={'schema_version','purpose','runtime_vault'} or d['schema_version']!=1 or d['purpose']!=purpose:raise B('bundle schema invalid')
 raw=d['runtime_vault']
 if not isinstance(raw,str) or len(raw)!=128:raise B('key length invalid')
 try:k=bytes.fromhex(raw)
 except ValueError as x:raise B('key encoding invalid') from x
 if len(k)!=64:raise B('key size invalid')
 return k,fsha(p)
def gitgate(sha):
...
def verify_host():
 if not MAPPER.exists():raise B('runtime mapper missing')
 if run(['blkid','-o','value','-s','TYPE',str(MAPPER)])!='ext4':raise B('runtime filesystem not ext4')
...
def status(require=False):
 try:r=verify_host()
  ...
  if require:raise
  return {'status':'absent-or-not-ready','validation':'FR06C4_RUNTIME_VAULT_NOT_READY','admission_opened':False}
 return r
def write_exclusive(p,v):
 ...
 try:os.write(fd,json.dumps(v,indent=2,sort_keys=True).encode()+b'\n');os.fsync(fd)
 finally:os.close(fd)
def plan(a):
 if os.geteuid()!=0:raise B('root required')
 gitgate(a.merge_sha)
 if IMAGE.exists() or MAPPER.exists():raise B('runtime vault already exists')
 ...
 free=shutil.disk_usage('/var/lib/aionex').free
 if free<SIZE+16*1024**3:raise B('insufficient free space plus reserve')
 if not 1<=a.ttl_seconds<=MAX_TTL:raise B('ttl invalid')
 t=now(); body={'schema_version':1,'subpart':'FR-06C4C1','operation':'provision-empty-runtime-vault','created_at':utc(t),'expires_at':utc(t+timedelta(seconds=a.ttl_seconds)),'nonce':secrets.token_hex(32),'merge_sha':a.merge_sha,'active_bundle_sha256':ad,'recovery_bundle_sha256':rd,'size_bytes':SIZE,'production_runtime_copy_permitted':False,'service_stop_or_restart_permitted':False,'bind_mount_install_permitted':False};body['plan_id']=digest(body);RUNTIME.mkdir(parents=True,exist_ok=True,mode=0o700);p=RUNTIME/f"plan-{body['plan_id']}.json";write_exclusive(p,body);return {'status':'planned','plan':str(p),'plan_id':body['plan_id'],'confirmation':'PROVISION-'+body['plan_id'][:16],'production_executed':False}
def loadplan(p):
...
def apply(a):
 ...
 STATE.mkdir(parents=True,exist_ok=True,mode=0o700); RUNTIME.mkdir(parents=True,exist_ok=True,mode=0o700); lock=os.open(RUNTIME/'operation.lock',os.O_RDWR|os.O_CREAT|os.O_NOFOLLOW,0o600);fcntl.flock(lock,fcntl.LOCK_EX|fcntl.LOCK_NB)
 tmp=KEYS/('apply-'+secrets.token_hex(6));tmp.mkdir(parents=True,mode=0o700);ap=tmp/'active';rp=tmp/'recovery';ap.write_bytes(ak);rp.write_bytes(rk);os.chmod(ap,0o600);os.chmod(rp,0o600);opened=False;mounted=False
 try:
  IMAGE.parent.mkdir(parents=True,exist_ok=True,mode=0o700);run(['fallocate','-l',str(SIZE),str(IMAGE)],600);os.chmod(IMAGE,0o600)
  run(['cryptsetup','luksFormat','--batch-mode','--type','luks2','--cipher','aes-xts-plain64','--key-size','512','--pbkdf','argon2id','--key-file',str(ap),str(IMAGE)],600);run(['cryptsetup','luksAddKey',str(IMAGE),str(rp),'--key-file',str(ap)],300);run(['cryptsetup','open','--key-file',str(ap),str(IMAGE),MAPPER_NAME],180);opened=True;run(['mkfs.ext4','-q','-L','AIOS06_RUNTIME',str(MAPPER)],300);mount_vault();mounted=True
  for sub in ('docker','containerd'):(MOUNT/sub).mkdir(mode=0o700);os.chown(MOUNT/sub,0,0);os.chmod(MOUNT/sub,0o700)
  header=RUNTIME/'container-runtime-vault.header';run(['cryptsetup','luksHeaderBackup',str(IMAGE),'--header-backup-file',str(header)],120);os.chmod(header,0o400)
  ready=verify_host(); receipt={'schema_version':1,'subpart':'FR-06C4C1','status':'empty_container_runtime_vault_provisioned_admission_closed','completed_at':utc(),'merge_sha':a.merge_sha,'size_bytes':SIZE,'mapper':str(MAPPER),'mount_root':str(MOUNT),'subpaths':['docker','containerd'],'active_bundle_sha256':ad,'recovery_bundle_sha256':rd,'header_staging_path':str(header),'header_sha256':fsha(header),'production_runtime_copied':False,'services_stopped_or_restarted':False,'bind_mounts_installed':False,'admission_opened':False,'cloudflare_changed':False,'ready_validation':ready['validation']};write_exclusive(STATE/'provision-receipt.json',receipt);return {'status':receipt['status'],'receipt':str(STATE/'provision-receipt.json'),'production_runtime_copied':False}
 except Exception:
...
def prove(a):
 ...
 if os.path.ismount(MOUNT):run(['umount',str(MOUNT)]);run(['cryptsetup','close',MAPPER_NAME])
 ...
 try:
  run(['cryptsetup','open','--key-file',str(rp),str(IMAGE),MAPPER_NAME]);mount_vault();verify_host();run(['umount',str(MOUNT)]);run(['cryptsetup','close',MAPPER_NAME]);run(['cryptsetup','open','--key-file',str(ap),str(IMAGE),MAPPER_NAME]);mount_vault();verify_host();out={'schema_version':1,'subpart':'FR-06C4C1','status':'independent_runtime_recovery_key_proved','observed_at':utc(),'recovery_open_passed':True,'active_reopen_passed':True,'key_material_persisted':False,'admission_opened':False};
  if (STATE/'recovery-proof.json').exists():(STATE/'recovery-proof.json').unlink()
  write_exclusive(STATE/'recovery-proof.json',out);return out
 finally:
  for x in (ap,rp):
   try:x.unlink()
   except:pass
...
def unlock(a):
 ...
 if subprocess.run(['systemctl','is-active','--quiet','containerd']).returncode==0 or subprocess.run(['systemctl','is-active','--quiet','docker']).returncode==0:raise B('Docker and containerd must be stopped before unlock')
...
def parser():
 ...
 st=s.add_parser('status');st.add_argument('--require-host-ready',action='store_true')
 for n in ('plan','apply'):
 ...
 q=s.add_parser('unlock');q.add_argument('--active-bundle',type=Path,required=True);q.add_argument('--confirmation',required=True)
 s.add_parser('wipe-inputs');return p
def main():
 a=parser().parse_args()
 try:
  if a.cmd=='status':o=status(a.require_host_ready)
  elif a.cmd=='plan':o=plan(a)
  elif a.cmd=='apply':o=apply(a)
  elif a.cmd=='prove-recovery':o=prove(a)
  elif a.cmd=='unlock':o=unlock(a)
  else:o=wipe()
