#!/usr/bin/env python3
"""Upload and fully read back the FR-06C2 database-vault LUKS2 header in private R2.

Designed to run inside the existing backup-worker container, where the private
R2 credential source is already mounted. It accepts only the one fixed header file copied into container tmpfs and never reads a LUKS key bundle.
"""
...
INPUT_ROOT = Path("/run/aionex-fr06c2-headers")
DEFAULT_CREDENTIALS = Path("/run/operator-secrets/r2-backup-source.env")
HEADER_FILES = {
    "database-vault": "aionex-database-vault.header",
}
ALLOWED_CREDENTIALS = {
    "R2_BACKUP_ENDPOINT",
    "R2_BACKUP_BUCKET",
...
def _upload_one(client: Any, bucket: str, key: str, role: str, path: Path) -> dict[str, Any]:
    ...
    metadata = {"sha256": checksum, "aionex-subpart": "fr-06c2b", "vault-role": role}
    try:
...
def upload(args: argparse.Namespace, client: Any | None = None) -> dict[str, Any]:
    ...
    for role, filename in HEADER_FILES.items():
        ...
        key = f"{prefix}/fr06c2/luks2-headers/{args.generation}/{filename}"
    ...
    return {
        ...
        "objects": objects,
        "object_count": 1,
        "references_distinct": len({item["reference"] for item in objects}) == 1,
