...
def unseal_all():
 for _,src,_,_ in reversed(PATHS):
  if exact_mount(src):subprocess.run(['umount',str(src)],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
...
def install_gates():
 for s,d in SYSTEMD:atomic_install(s,d)
 run(['systemctl','daemon-reload']);run(['systemctl','enable','aionex-fr06c5-host-state-bind.service'])
def remove_gates():
 subprocess.run(['systemctl','disable','aionex-fr06c5-host-state-bind.service'],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
 for _,d in SYSTEMD:
  try:d.unlink()
  except OSError:pass
 subprocess.run(['systemctl','daemon-reload'],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
def stop_watchers():
 states={u:active(u) for u in ('aionex-runtime-watch.timer','aionex-runtime-watch.service')}
 subprocess.run(['systemctl','stop','aionex-runtime-watch.timer','aionex-runtime-watch.service'],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
 return states
def restore_watchers(states):
 if states.get('aionex-runtime-watch.timer'):subprocess.run(['systemctl','start','aionex-runtime-watch.timer'],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
 if states.get('aionex-runtime-watch.service'):subprocess.run(['systemctl','start','aionex-runtime-watch.service'],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
def quiesce_restart_policies(t):
 for r in t['containers']:
  run(['docker','update','--restart=no',r['id']],60)
 for r in t['containers']:
  d=json.loads(run(['docker','inspect',r['id']]))[0];rp=(d.get('HostConfig') or {}).get('RestartPolicy') or {}
  if str(rp.get('Name') or 'no')!='no':raise B('container restart policy did not quiesce')
def restore_restart_policies(t):
 for r in t['containers']:
  run(['docker','update','--restart='+r['restart'],r['id']],60)
def stop_live(t):
...
def rollback_prestart(t,watch):
 subprocess.run(['systemctl','stop','docker.service','docker.socket'],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
 subprocess.run(['systemctl','stop','aionex-fr06c5-host-state-bind.service'],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
 subprocess.run(['python3',str(ROOT/'scripts/security/fr06c5_host_state_bind.py'),'rollback'],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
 remove_gates();unseal_all();subprocess.run(['systemctl','start','docker.socket','docker.service'],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL);time.sleep(2);subprocess.run(['docker','start',*[r['id'] for r in t['containers']]],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL);restore_watchers(watch)
def acceptance(t):
...
def plan(a):
 ...
 gitgate(a.merge_sha);evidence(a.evidence.resolve(),a.merge_sha);c5gate();t=topology()
...
def apply(a):
 ...
 STATE.mkdir(parents=True,exist_ok=True,mode=0o700);fd=os.open(STATE/'operation.lock',os.O_RDWR|os.O_CREAT|os.O_NOFOLLOW,0o600);fcntl.flock(fd,fcntl.LOCK_EX|fcntl.LOCK_NB);started=False;watch={}
 try:
  reject_nested_mounts();precopy();watch=stop_watchers();quiesce_restart_policies(t);stop_live(t)
  for _,src,_,_ in PATHS:seal(src)
  manifests=exact_copy_and_manifest();bootstrap_match();hidden_fd_count=require_zero_hidden_underlay_fds();install_gates();run(['systemctl','start','aionex-fr06c5-host-state-bind.service'],120)
  run(['systemctl','start','docker.socket','docker.service'],180);started=True;run(['docker','start',*[r['id'] for r in t['containers']]],180);restore_restart_policies(t);restore_watchers(watch);n=acceptance(t)
  ...
  if not started:
   try:rollback_prestart(p['topology'],watch)
   except Exception as rb:raise E('host-state cutover failed before candidate start and rollback failed; control tunnels remain independent') from rb
   raise E('host-state cutover failed before candidate start; legacy runtime restored') from original
  raise E('host-state cutover acceptance failed after candidate containers started; candidate state is authoritative and blind rollback is prohibited') from original
