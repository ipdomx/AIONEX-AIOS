...
'''
    ...
    def _test_universal_source(cls, source: Path, project: str) -> dict[str, Any]:
        ...
        if infra_dockerfile.is_file():
            ...
            if (
                "USER app" not in dockerfile
                or ":latest" in dockerfile
                or not re.fullmatch(r"FROM [^\s]+@sha256:[0-9a-f]{64}", first_line)
            ):
                findings.append("infrastructure container target is not least-privilege and immutably pinned")
