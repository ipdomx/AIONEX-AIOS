...
def infer_project_profile(objective: str) -> ProjectCapabilityProfile:
    ...
    tech = {
        "web": "Next.js 16.2.11 / React 19.2.3 security-patched production target; dependency-free local preview",
        "api": "FastAPI 0.141.1 / Uvicorn 0.52.1 / Pydantic 2.13.4 domain API target",
        "mobile": "Expo SDK 57 / React Native New Architecture source target",
        "desktop": "Tauri 2 capability-scoped desktop target",
        ...
        "infrastructure": "Python 3.14.6 OCI container + Compose/IaC least-privilege baseline",
        "smart_contract": "Solidity contract source with deployment disabled by default",
...
def augment_universal_project(
    ...
    if "api" in profile.targets:
        files.update(_api_target(domain))
...
def _api_target(domain: Mapping[str, Any]) -> dict[str, str]:
...
def create_resource(entity_name: str, payload: ResourcePayload) -> dict[str, Any]:
    values = _validate(entity_name, payload.data)
    with sqlite3.connect(DATABASE) as connection:
...
def list_resources(entity_name: str) -> dict[str, Any]:
    _entity(entity_name)
    with sqlite3.connect(DATABASE) as connection:
...
def delete_resource(entity_name: str, record_id: int) -> dict[str, bool]:
    _entity(entity_name)
    with sqlite3.connect(DATABASE) as connection:
        cursor = connection.execute(
...
def hash_password(password: str, salt: bytes | None = None) -> tuple[str, str]:
    ...
    selected = salt or secrets.token_bytes(16)
    digest = hashlib.pbkdf2_hmac("sha256", password.encode("utf-8"), selected, 200_000)
    return selected.hex(), digest.hex()
...
"""
    return {
        "targets/auth/service.py": service,
        "targets/auth/README.md": "# Authentication target\n\nLocal password hashing/session boundary. Production identity federation, MFA, recovery and email verification are activated only through reviewed provider/configuration gates.\n",
    }
...
def _mobile_target(
    ...
    entities_literal = json.dumps(
        [str(item["label"]) for item in domain.get("entities", [])],
        ensure_ascii=False,
    )
    workflows_literal = json.dumps(
        [str(item["name"]) for item in domain.get("workflows", [])],
        ensure_ascii=False,
    )
    screen = (
...
def _infrastructure_target() -> dict[str, str]:
    dockerfile = """FROM python:3.14.6-slim-bookworm
