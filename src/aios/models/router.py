...
from .local import UnconfiguredLocalProvider
...
class ModelRouter:
    def build_default(self) -> ModelProvider:
        ...
        model = os.environ.get("AIOS_MODEL_NAME", "unconfigured")
        ...
        if provider == "local":
            return UnconfiguredLocalProvider(model)
        raise RuntimeError(f"Unsupported legacy model provider: {provider}")
