...
This source-only stage consumes the accepted C4B1 writer receipt, C4B2 runtime/backup-drain receipt, and one C4B3 cleanup candidate that requires removal of an owned staging name. It rechecks the exact current backend/studio-worker/backup-worker epoch and Studio volume identity before touching candidate paths.
...
The host scan examines every numeric process thread for fd, cwd, root, exe, and mmap references to that exact staging device/inode. It performs two complete scans and revalidates the staging/final identity between and after the scans. Ambiguous disappearing proc entries, permission failures, writer/reader epoch drift, unexpected Studio mounts, initializer activity, path/identity drift, or any visible reference block the receipt.
...
Final local acceptance on this candidate: 25/25 focused process-reference tests passed and the complete repository suite passed 1872/1872. The earlier stacked-draft acceptance also recorded Ruff success. The current operator shell does not expose a Ruff executable, so no additional Ruff rerun is claimed after this documentation-only acceptance update; the full pytest acceptance below is rerun on the final tree before commit.
