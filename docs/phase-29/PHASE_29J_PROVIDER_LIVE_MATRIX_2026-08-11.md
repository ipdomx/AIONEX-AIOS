...
## Fireworks AI runtime closeout — 2026-08-12

After the Fireworks account was restored to active billing state, the existing production `FIREWORKS_API_KEY` was revalidated against the official Fireworks model inventory and the prior HTTP 412 account-suspension blocker cleared. A disposable AIOS agent then executed `accounts/fireworks/models/gpt-oss-120b` through the durable `run_job` runtime path, returned the bounded acceptance response, recorded 153 total tokens and persisted latency/last-used state, and transitioned the provider from `error` to `connected`. The remote Fireworks inventory was used only for provider verification and was not imported into the AIOS model catalog; hosted Kimi, DeepSeek, and other third-party models remain external inventory unless separately approved. The disposable job and agent were removed after acceptance; provider usage/state and append-only audit evidence remain durable. No credential or response secret is recorded here.
