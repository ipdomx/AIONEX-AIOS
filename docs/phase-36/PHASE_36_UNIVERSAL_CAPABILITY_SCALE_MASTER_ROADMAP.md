...
> **2026-09-17 FR-06C5D5 source contract:** canonical `docs/project/PLAN.json` under `FR-06.host_state_cutover_admission` extends the partial authority to external notification dispatch with schema 4. One admitted claim commits its ownership and started attempt before provider I/O; a committed single-use transition prevents replay. Cancellation, uncertain provider outcomes and failed cleanup or result commits retain independent evidence, while business completion, notification creation, in-app delivery and stored backlog continue during dispatch closure. Project, backup and academy scope compatibility is retained. The deployed notification app remains baked into its image. Source acceptance does not establish deployment coverage or full-host drain; current evidence remains exclusively in the canonical project hub.
