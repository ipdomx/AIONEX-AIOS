...
> **2026-09-17 FR-06C5D4 source contract:** canonical `docs/project/PLAN.json` under `FR-06.host_state_cutover_admission` extends admission to academy course packages with schema 3 and durable build ownership through publication, cleanup and confirmed business commit. Legacy or uncertain builds cannot be reclaimed by age, and frozen unstarted backlog is distinct from active work. Project and backup scope compatibility is retained. The deployed academy app is baked into its image while the course factory source is visible through the workspace bind. Source acceptance does not establish deployment coverage or full-host drain; current evidence remains exclusively in the canonical project hub.
