...
- Verification: durable authority `8/8 PASS`; focused route-plan + durable authority `15/15 PASS`; current unified route-plan/durable/Phase29J/shipped-head regression `23/23 PASS`; migration round-trip `0029 -> 0028 -> 0029` with route-table count `4 -> 0 -> 4`; full Backend static gate PASS with Mypy `181` files; fresh Full Backend `657 passed, 0 failed` in `106.59s` on PostgreSQL 16 + Redis 7 disposable. No connection exhaustion/deadlock/PANIC/FATAL and all durable route rows are cleaned after tests.
