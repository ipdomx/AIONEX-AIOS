...
##### Stage 4A — governed single-speaker STT source candidate

- The candidate pins `gpt-4o-mini-transcribe-2025-12-15`, accepts only checksum-verified finite PCM WAV, uses provider JSON, caps size/duration at `20 MiB / 10 minutes`, and stores the official `$0.003/minute` estimate separately from the exact Owner maximum. Per-request actual cost remains unknown unless authoritative usage is returned.
- Provider-neutral transcript/caption contracts keep raw transcript and storage locators private; public snapshots expose hashes, timing, language, counts and pseudonymous `speaker-NNN` keys only. Deterministic WebVTT/SRT and a hash-only manifest are included. Dubbing planning preserves timing/speaker scope but stays blocked on translation, per-segment stock TTS, timing fit and final local master evidence.
- Alembic `0036` and the durable authority add planned-to-armed admission, one-attempt cost control, source integrity evidence, lease/fencing, pre-HTTP `submitting`, ambiguity-to-review with no auto-resubmit, transcript package storage, Studio revision and redacted public evidence. The permanent `audio-transcript-worker` remains non-root and hard-disabled by default.
- Only `governed-stt-transcript=source_built` is added at this source checkpoint. Provider requests/spend remain `0 / $0.00`; multi-speaker diarization, dubbing completion, music/vocals/SFX and voice transformation/clone remain separate gates.
- Final source evidence: free pinned-model lookup PASS with zero transcription/spend; focused Root `35/35`, Core `771/771`, transcript authority `27/27`, affected Audio/Media `89/89`, clean `0036 -> 0035 -> 0036`, complete Backend `871 passed / 1 skipped` at `65.36%` coverage, full static/security/reporting gates, and exact Production candidate image `2fc8df77...` no-network disabled smoke PASS. Backend evidence SHA-256 `195df859cd0db4b5dcd1cf6dcd74f43370032f8c870d56d3a148cb22d0729c80`; candidate evidence SHA-256 `da4b03d76013131bfa76c79dc5696e2c8c6c1d779889ff153bf4ed14477a83f4`. Production remains `0035`; no STT request or Stage 4 deployment has occurred.

Next safe gate: complete protected validation, disabled Production deployment, and one separately bounded single-speaker STT canary with full checkpoint/cleanup evidence.
