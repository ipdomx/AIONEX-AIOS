...
- Checkpoint 4B1 establishes the governed local derivative engine: exact Sharp `0.35.3` on Node 24 is isolated inside the dedicated media-worker image, source/output checksums and raster dimensions are independently verified, actual transparency fails closed for JPEG, and a real synthetic PNG -> WebP image-level transform passes. Focused derivative tests `4/4 PASS`; no provider spend, schema change or Production worker restart is introduced.
