...
- Maturity truthfulness: `distributed-project-execution` and `thousand-user-admission` are `locally_executed`; horizontal worker scaling remains `source_built`; Batch 36B remains `in_progress` until controlled production backup/deploy/live acceptance completes.
...
- Next safe transition: same-host scale/live acceptance is complete -> record/fix P36-0008 PostgreSQL healthcheck source without recreating the healthy database solely for log noise -> rerun Phase36/Compose/security gates -> finalize report/maturity -> protected PR before any transition to 36C.
...
- Resolution plan: update both production Compose copies to invoke `pg_isready` with the configured PostgreSQL user and database; add a source regression contract. Do not recreate a healthy production PostgreSQL instance solely to remove low-severity healthcheck log noise; apply the corrected healthcheck on the next controlled PostgreSQL recreate/maintenance.
...
- Production rollback/activation boundary: no PostgreSQL runtime change is required for this source correction during the current 36B activation.
