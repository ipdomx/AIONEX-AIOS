...
> **2026-09-17 FR-06C5D6 source contract:** canonical `docs/project/PLAN.json` under `FR-06.host_state_cutover_admission` extends partial authority to security remediation preparation with schema 5. The dedicated producer retains admission through its business commit; a committed claim and single-use begin own one immutable-input copy thread through atomic source-plus-plan publication, strict cleanup and result settlement. Dedicated preparation proof survives later patch/retest metadata updates. Legacy or uncertain preparation remains visible, and pristine planned work stays frozen. Earlier project, backup, academy and notification scope compatibility is retained. Entrypoint root preparation and health writes remain declared control-plane effects; the deployed app remains baked into its image. Source acceptance does not establish live deployment coverage or full-host drain; current evidence remains exclusively in the canonical project hub.
