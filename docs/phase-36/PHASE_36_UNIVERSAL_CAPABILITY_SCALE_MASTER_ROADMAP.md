...
- **Stage 3C multi-scene exit-gate source/test candidate:** adds budget-capped project arming, latest-per-target project snapshots and idempotent terminal-scene replacement while preserving failed execution evidence and rejecting unreconciled ambiguous submissions. Synthetic acceptance proves `$2.39` rejects the current `$2.40` four-scene plan before mutation, an exact cap arms all scenes without HTTP, an expired worker lease resumes the same provider job without duplicate submit, and a terminal failure is replaced only for the affected scene; after recovery the existing Phase36D FFmpeg assembly completes and materializes the next Studio revision while the other three scene targets are reused unchanged. Disposable Alembic-to-0034 PASS; new suite `4/4`, expanded Phase36F/Phase36D regression `54/54`, Ruff/Mypy/compile/diff PASS. No migration, provider HTTP/spend or Production mutation. Next: protected CI/merge plus disabled/no-spend Production validation before any real multi-scene spend.
