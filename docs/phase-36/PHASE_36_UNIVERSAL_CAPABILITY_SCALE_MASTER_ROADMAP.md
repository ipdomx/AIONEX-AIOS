...
> **2026-09-17 FR-06C5D7B1 ZAP observation subpart:** canonical `docs/project/PLAN.json` records strict counter/identifier/inventory validation and a fresh passive-queue observation after the last root or targeted active scan. Missing or malformed replies are not zero work or successful empty results. This is NOT durable remote ownership, shared-daemon exclusivity, remote stop/cleanup proof, scanner-worker fencing or deployed coverage. D7B and D7 remain incomplete; the canonical project hub records actual tests, merges and next work.

> **2026-09-17 FR-06C5D7A request-only source contract:** canonical `docs/project/PLAN.json` under `FR-06.host_state_cutover_admission` extends schema 5 to schema 6 with `security_scan_requests` only. Dedicated scan creation and remediation retest retain shared admission through the caller transaction, before policy setup, DNS and the retest row lock. Migration preserves closure, operation identity and previous activity evidence. D6 preparation ownership remains compatible. Scanner worker claims, execution, cancellation, retries, processes, threads and remote ZAP settlement are NOT covered by D7A; D7 remains incomplete. No source test or merge establishes production activation, full-host coverage or drain. Current acceptance and remaining work stay exclusively in the canonical project hub.
...
- **Stage 3B Sora reference transport merged/deployed disabled + image/logo/reference live acceptance PASS:** protected PR #452 merged as `2d632c75edaf427129e7974bb867c789520ac23b` and deployed only the hard-disabled Video Worker image `sha256:9a561b195bcfb9d048633f424a08e14d75baa405a7a81ab92288f8cacb2f479d`. Image-to-video passed one `$0.40` 4s/1280x720 submit after 11 polls (evidence `bd3234b3e06a62082d474c46d209c6c267b894710a7d2c2b434357f480eb2f52`); logo-to-video passed a separate one-submit `$0.40` canary after 14 polls (evidence `c500909a52f14ce0089f77dc58b70a54ddd36c521bdb9a56905baa8b2bb71719`) and docs PR #454 merged as `1661599d026e788cc934dbe6f5db51ad55dc47e3`. A separate no-spend reference precheck then passed, followed by one `reference-to-video` submit (`reference_count=1`, `max_attempts=1`) completing after 10 polls; FFmpeg/ffprobe accepted a 1,990,870-byte MP4, actual cost `$0.40`, input/output objects and all synthetic rows were deleted/verified, persistent worker stayed disabled with cycles/errors `0/0`, global VideoExecution rows `0`, `/ready` `10/10`. Sanitized reference evidence SHA-256 `ba862cf6d437e1ab058a05398f67cde9195ba4ac254ee6990e1850617e16306a`. Current Production pipeline accepts exactly one governed reference even though the generic Factory contract permits up to three; 2–3 reference execution is not claimed. Next Sora gate: protected source-first multi-scene advertisement + worker-failure/selective-resume + final FFmpeg assembly exit evidence; Gemini/Veo remain separate provider gates.
- **Stage 3C real Sora multi-scene text-to-video + recovery + final assembly PASS:** PR #456 merged the budget-capped project coordinator as `a8c720878fe976a81ffa20b2b892b0df85b38a15`; disabled Production validation proved `$2.39` reject / `$2.40` exact-cap arm with zero requests/spend and evidence `f44501862a5916d3cfa313883eca7c997778fa8956a2cf33da0a93483d63d296`. Its docs PR #457 merged as `b428dc922e684ed9d63088b4e7e521cd6e618c79`. A real four-scene `sora-2` text-to-video acceptance then completed `4s/8s/8s/4s` at exact total `$2.40`, each scene `attempts=1`. The opening worker-crash drill recovered fencing `2 -> 3` against the same provider-job hash with `duplicate_submit=false`; all scenes completed, Phase36D assembled a `3,702,608`-byte MP4, Studio advanced to revision `2`, `5/5` objects and all synthetic rows were deleted, global VideoExecution returned to `0`, and the persistent worker remained hard-disabled/Healthy. Live evidence SHA-256 `2622663ed44359f95a2e5d12660b273965e0086cb672eeb94a4079df69b79e40`. Next: separate bounded **logo + brief -> real multi-scene advertisement** acceptance; text-to-video success alone does not close 36F.



#### Stage 3D — logo + brief final exit acceptance and 36F closeout candidate (2026-08-21)

- The first Stage 3D one-shot had already created exactly four completed `sora-2` jobs for the governed `opening/value/proof/close` prompts during `2026-08-21T13:33:34Z..13:35:59Z`, but its final result/evidence artifact was not retained and the empty closeout placeholders could not support a maturity transition. The precise terminal-output loss mechanism is not provable from the missing artifact, so no narrower cause is claimed.
- Recovery was fail-closed and **read-only at the provider boundary**. The recovery adapter overrides provider `submit` and `reconcile` as forbidden operations, lists/retrieves the prior jobs, and accepts a job only when its creation window, model, duration, size, exact compiled-prompt SHA-256 and previously observed provider-job hash all match one unique governed scene. The four pre-existing hashes were `1c67ffc00b52bf8c`, `3141d94944ed3d60`, `85bf93a748029623`, and `ea73b1269df86c4b`; no raw job ID, prompt, credential, authorization header, signed URL or storage key is retained in evidence.
- The recovered jobs were attached to a fresh tenant-scoped `logo-to-video` DAG only after the same deterministic `1280x720` AIONEX logo (`20,475` bytes; SHA-256 `178dfdfc71b1a0321fdacfe5703b495191e51ed5f85804ed3d03558cb31b9c69`) and brief had reconstructed the exact four scene prompt hashes. `new_provider_submissions=0`, `new_provider_reconciliations=0`, and recovery provider spend was `$0.00`; the already-incurred historical fixed-second cost remained exactly `$2.40` for `4s/8s/8s/4s`.
- All four prior outputs were downloaded once, passed the shipped FFmpeg/ffprobe 9.0 QA and completed as governed Media DAG provider nodes. Scene evidence before cleanup: opening `3,241,238` bytes / `433b23d20b6ba5ccec3fce93dca7ed1fd2629a026679f4a599bbc487857972a4`; value `3,844,734` / `f4454cb597bb58a08f606ac966f24d9cb533ff09118d653f817908d967f1042b`; proof `3,923,319` / `a185ec1e7654c131b597004217e2f0165b6fce10833acf53acdddedc76f7ebf4`; close `2,075,726` / `649874ac55f2ae278bacb99806fd9c31bc1eaf772f19044b28061331e304cf8a`.
- The existing Phase 36D worker assembled the real multi-scene advertisement, completed the graph/project snapshot and advanced the bound Studio asset to revision `2`. Final MP4 evidence before cleanup: `4,342,119` bytes, SHA-256 `ddb064ae9abf7679a12978301da62130c50c431546c306f699ba4780c1c80b18`.
- An atomic `validated-before-cleanup` checkpoint was persisted before destructive cleanup. Cleanup then deleted and verified all `6/6` synthetic objects (logo, four scenes, final assembly), removed all synthetic VideoExecution/Media graph/node/Studio rows and returned global VideoExecution to `0`. Post-health kept Backend/Media/Image/Derivative/Video/PostgreSQL Healthy, the persistent Video worker hard-disabled with cycles/errors `0/0`, `/ready` `20/20`, zero critical hits and zero provider jobs created during or after recovery.
- Sanitized evidence hashes: pre-cleanup checkpoint `99f4f6517a21f86cff2433b13c62b29d529ca54adabf020f904f517b85a32da6`; final live evidence `18bc5397dcec56e22e4a71fc7ba3f4cb50a2225e3bd13a32d624484142ed2aa3`; post-health `fee4e4581e579391045b45355553a8b36d046569c007b3080a663f3553feeb41`; recovery script `0211274f64b3b082eeb90fd7ce669a71d487f92957edc3b71799ad64594e0ae2`.
- Both 36F exit clauses are now evidenced: a logo + brief produced a real multi-scene advertisement through final Studio materialization, and Stage 3C independently proved same-provider-job worker recovery plus selective failed-scene replacement without re-rendering unaffected scenes. This closeout therefore promotes the live-accepted generation/multi-scene/continuity capabilities to `runtime_verified`, marks `36F=complete`, and opens `36G=in_progress` after protected merge and Production registry activation.
- Maturity remains deliberately granular: `video-final-export` stays `source_built` because the live provider/assembly acceptance was 720p and does not runtime-verify the advertised 1080p/1440p/4K range; `cinema-motion-vfx` stays `specified`; multi-reference 2–3 execution and Gemini/Veo remain unaccepted future gates. Closing 36F does not convert these unverified surfaces into production claims.
- **36F protected closeout activated:** PR #459 passed all required gates and merged as `a28a437c77e85362220ea0d069a79c21aa0a1504`. Production fast-forwarded from `ee6dd4da8fc72ee73a2037a78a241cc2d4329906`; a Backend-only restart reloaded the registry so actual HTTP now reports `36F=complete`, `36G=in_progress`, `current_batch=36G`. All other service identities remained unchanged/Healthy, Alembic stayed `20260819_0034`, active queues stayed zero, the persistent Video worker remained hard-disabled, readiness passed `20/20`, and activation incurred zero provider jobs/spend. Sanitized activation evidence SHA-256 `3349d614deb45f775fc59013e51f44e83ddd233ecd153c0682e14be51c7174bc`.
...
- **Stage 2 status activation:** PR #464 passed every protected gate and merged as `c5f07e5c2fcddd187ef5563749e3b70fd8a221d7`. Production fast-forwarded from `998fab35cee414d495050c352ff43667c2cff3ae`, built Backend image `90f278da...`, retained rollback image `7a821d47...`, and recreated Backend only. The live Backend registry/HTTP now expose the granular maturity decision; all other checked service identities remained unchanged, Alembic stayed `0034`, queues stayed zero, readiness passed `30/30`, and provider activity stayed `0 / $0.00`. Final activation evidence SHA-256 `98b36ed7a771485ca91dbbf1ada91d3ca659f697525cd19a86d5e747696e2d8c`.
- **Truth-boundary correction:** PR #465 merged as `a9bd80c7acb414f39e4f1307d304cf28657f9839` and was already active when independently checked. The live title now excludes generated SFX and is bounded to cleanup/alignment/mixing/mastering/local export. No duplicate restart was performed; queues stayed zero, readiness passed `30/30`, and observed-activation evidence SHA-256 is `65a699eb93d8491029b9684382c2adf6637f6d7df76847b239676aa0e2d48e10`.

##### Stage 3A — pinned stock-voice TTS source/isolated candidate

- Owner approval selected one OpenAI stock-voice route only: pinned `gpt-4o-mini-tts-2025-12-15`, built-in voice, provider PCM wrapped into governed WAV, short input, `20s` duration cap, `max_attempts=1` and conservative `$0.05` operator cap. Two credential-specific free model lookups confirmed exact model access; billable generation/spend remain `0 / $0.00`. Preflight evidence SHA-256 `9c2ea19a39c4ad1fd3aab731012acbfe14a2132000082e5e8edb79fefbaa809a`.
- Candidate adds Alembic `0035`, durable arm/cost/lease/fencing/submission-state authority, a non-root profile-gated Worker with `AUDIO_SPEECH_LIVE_ENABLED=false`, exact provider transport, and `stock speech -> local cleanup/master/waveform/export`. A synchronous ambiguous submission is failed without automatic resubmit; custom voice/clone/music/SFX are rejected.
- Final isolated source evidence: provider/worker `20/20`, AudioFactory `12/12`, durable runtime `12/12`, affected PostgreSQL/Redis `58/58`, clean Alembic `0035 -> 0034 -> 0035`, complete Core `761/761`, complete Backend `841/841` with `65.21%` coverage, full Ruff/Mypy (`212` files), Compose/YAML and real current-image disabled no-network smoke PASS. Database/cache critical hits were `0/0`; Backend metadata SHA-256 `d9a1b6d1a0f8d02caa11fd9c8861ca958bdc6baae6725bed8f968b112b2582a5`. Production is untouched; no maturity advances from source evidence.

- **Stage 3 disabled deployment and first live boundary:** PR #467 merged as `2cafe0243a81e51c6502655fd33bf93eb57c8b90`; backup/restore, Alembic `0035`, Backend recreation and hard-disabled Audio Speech Worker all passed with zero deployment requests/spend. The first live one-shot made exactly one bounded provider request but rejected the streamed WAV locally as `provider_audio_duration` before storage or downstream rendering. Exact cost remains unknown within the approved `$0.05` upper bound; failed synthetic rows were cleaned to zero.
- **P36-0027 PCM correction activated:** protected PR #468 merged as `887ec9f289bab85b1794514c6a33763d69f69b62`; Backend and the hard-disabled Audio Speech Worker alone moved to image `fc3f1aa9...`, Alembic remained `0035`, all non-target service identities stayed unchanged, and zero provider requests/spend occurred during activation. PCM activation evidence SHA-256 `a449eb3314b0aad84ab9a5dba2944a9b8efe34b3f7a56340daaac8153be7faab`.
- **Stage 3 corrected stock-voice live acceptance PASS:** one new `max_attempts=1` request used pinned `gpt-4o-mini-tts-2025-12-15` and built-in `marin`. Validated 24 kHz mono PCM became a finite `271,244`-byte WAV, then the persistent Media Worker produced a `1,084,878`-byte 48 kHz stereo WAV, real waveform, `36G.audio-qa.v1` PASS and Studio revision `2`. Cleanup deleted/verified `5/5` objects and all synthetic rows; queues returned zero, the persistent speech worker stayed disabled, and readiness passed `30/30`. Exact cost remains unknown within the request's `$0.05` upper bound. Consolidated evidence SHA-256 `0cdbf4baafda5d16938fb6eb473ab6064befc8a3cbe933e3b394dbb471986ebf`.
- **Truthful total accounting and maturity:** Stage 3 used two provider audio requests total—the rejected pre-fix response and the successful corrected acceptance—each with `max_attempts=1`; automatic retries/duplicates were `0/0`, exact total cost is unknown, and the combined approved upper bound is `$0.10`. Accounting evidence SHA-256 `63990878badaea35e3bdf0c4bcb405c85cac2f4efb393bd8f969872efc080623`. Only granular `stock-voice-tts` advances to `runtime_verified` with synthetic-voice disclosure; aggregate STT/TTS/dubbing and podcast remain `source_built`, song/voice transformation remain `specified`, and 36G stays `in_progress`.

##### Stage 4 — governed single-speaker STT Production acceptance

- Protected PR #470 merged as `7138a9f4882dc48d219d3a9a8343f6cc27170cfe`; backup/restore PASS preceded Alembic `0036`. Backend alone moved to image `sha256:203a9ed8d4cd51cc0959f1634b5f566988da803c836249d8aa4fcc193d7a0fb2`, and the permanent Transcript Worker started hard-disabled. Disabled evidence SHA-256: `9750fd1c9b0002494e478e3e38597a7cea9862e7fa4579761620324355ff1333`.
- One locally generated `5.24449s` single-speaker WAV used pinned `gpt-4o-mini-transcribe-2025-12-15`, `max_attempts=1`, estimate `$0.0002622` and cap `$0.005`. Exactly one transcription request completed; actual cost remains unknown without authoritative usage. Private transcript, WebVTT, SRT, manifest and Studio revision `2` passed; cleanup deleted/verified `5/5` objects and all synthetic rows.
...
- Only granular `complete-stock-voice-dubbing=source_built` is added; aggregate dubbing remains `source_built`, and music/vocals/SFX/voice transformation/clone remain separate gates.
...
- Protected PR #477 merged as `092314b8ba05e63809e6c258aac3e80502ec2d7d`; backup/restore passed, Alembic advanced to `0038`, and exact Backend plus a permanent hard-disabled Music Worker became Healthy with zero activation requests/spend.
...
- Final security-corrected immutable staged-tree snapshot `6f7a0fefc2ab39bf76610f36c894fc599805e7f4` passed root `78/78`, focused CodeQL remediation `27/27`, full Backend `979 passed, 1 skipped`, Alembic `0039 -> 0038 -> 0039`, Ruff/Mypy/AST/diff/Compose, explicit health Secret Sink scan, zero residual Song/Media/Studio rows and a real FFmpeg 9 synthetic-boundary acceptance producing separated lyrics/composition-vocals/stems/mix/master/final hashes, QA PASS and Studio revision `2` with zero Provider/GPU spend. Frozen gate SHA-256 `425d8f72da6f6316cb2d696b7e6334e7b51d71c7e2bbc2fa17a635b9f7a0b059`.
...
- Production preflight after 0042 migration found the fresh course-package volume ownership would conflict with a direct UID-1000 worker. The protected bootstrap candidate now passes a real fresh-volume proof: `755:0:0 -> 700:1000:1000`, runtime uid/gid `1000:1000`, write succeeds, second bootstrap preserves inode/mtime, and the process receives no network; worker activation remains held until the hotfix merges.
...
- 36H.6B isolated PostgreSQL/Redis runtime acceptance passed: 1,000/1,000 durable admissions and grant consumptions across 10 tenants, 1,000 connected presence sessions, 250/250 failed-node presences reaped and fenced recovery completed, Redis delivered 1,000/1,000 events with zero tenant leaks/duplicates/failures and zero stale subscribers after one hub loss. Attempt 1 exposed a real Redis listener-before-subscribe lifecycle defect; source was fixed and Attempt 2 passed. LiveKit/TURN/Egress scale and production migration remain unclaimed.
- Part 6C source adds fail-closed TURN fallback and recording failover evidence plus a production prerequisite evaluator. Isolated evidence passes.
- Production Gate A applied the already-merged linear `0039 -> 0040 -> 0041` migrations after a verified private pg_dump/pg_restore-list backup; all four realtime authority tables are present and empty, while Backend/PostgreSQL/Redis/Song Worker stayed healthy with zero restarts.
- Production Gate B isolated runtime preflight pulled and digest-verified LiveKit `v1.13.5`, Coturn `4.17.2`, and Egress `v1.13.0`; Attempt 2 ran all three against disposable Redis on an internal Docker network with no published host ports. The preflight also found and fixes the Coturn source-only `cap_drop: ALL` exec boundary by retaining only `NET_BIND_SERVICE`. Public activation remains blocked by missing dedicated realtime/TURN DNS+trusted TLS, unopened public media ports, no public TURN/SFU/recording acceptance, and the host's observed 1 Gbps NIC being below LiveKit's current 10 Gbps-or-faster production recommendation for high-scale media.
- 36I.2 local 2D runtime acceptance passed: deterministic animation/game projects produced checksum-addressed manifests; headless Chromium proved frame progression and keyboard interaction with zero external requests/console errors, and a real 1.72-second VP8 preview was FFprobe-validated. No provider/GPU spend or production/tunnel mutation occurred.
- Production route-wiring source replaces the remaining process-local `AIRealtimeHub` notification/websocket path with the tested tenant-hashed Redis backplane, owns lifecycle after Redis startup, and removes the obsolete singleton. Live media remains independently blocked by DNS/TLS/public-network acceptance.

Implementation ledger — 2026-08-25 / 36I Part 5 final exit:
- Added governed FFmpeg 9 chroma-key/overlay VFX compositing to the existing Media DAG runtime with bounded, injection-safe parameters and QA; isolated real execution passed with H.264 1280x720/30fps/90-frame evidence and zero network/provider use.
...
### P36-0022 — Retained Hunyuan3D production image failed exact pre-launch vulnerability acceptance — 2026-08-31

- Batch: 36N comprehensive pre-launch closeout continuation.
- Environment/component: retained RunPod Hunyuan3D 2.1 PBR runtime image, exact Production digest `sha256:34bd37c577a8c769005a11f94bf4658d0b9f31d52df5c75e2a8f01a5ed8499dc`; no GPU job was submitted by this audit.
- Visible symptom/user impact: the historical Phase 34C image remained functionally accepted but had not been re-certified against the current vulnerability database. Exact Trivy `0.73.0` image analysis reported `33 Critical / 590 High`; therefore Hunyuan cannot remain an eligible live provider until a new hardened image passes both security and functional/PBR acceptance. TripoSR remains available as the approved fallback.
- Detection/reproduction evidence: exact digest scan retained outside Git at `.deployment-backups/phase36n-comprehensive-closeout/20260831T005725Z/image-scans/hunyuan3d-v11.json`, SHA-256 `5d392578b584d94adaaf995b5f1de8d5e0afb7ef6457b29183e04d7b388a6d06`; compact source-controlled summary `docs/phase-36/evidence/36N-2026-08-31-comprehensive-closeout-summary.json` SHA-256 `6eadc40ffb314b03970dfa546a74229e397f722002ffa8afe924786ea432f0c7`.
- Root cause: the accepted GPU image was derived from an old general-purpose RunPod development base containing obsolete OS/build/Jupyter/SSH packages, and its active inference venv also retained security-sensitive versions that aged after the original functional acceptance. Functional reproducibility and immutable pinning prevented runtime drift but did not provide future vulnerability immunity.
- Why prior tests/architecture did not prevent it: the Phase 34C acceptance correctly proved PBR output integrity, deterministic generation and scale-to-zero behavior at that time; later Phase 36 source scanners did not inspect the exact 97.5 GB retained GPU image. The initial closeout image scan also targeted the older local v4 tag before the exact v11 production digest was re-identified and scanned.
- Technical detail: `linux-libc-dev` in the development base accounts for `501` OS High/Critical findings, but this is not dismissed as build-only noise because the active `/opt/hunyuan-venv` independently intersects `35` High/Critical findings, including Critical advisories affecting `torch 2.5.1+cu124` and `pytorch-lightning 1.9.5` plus High findings in Pillow, Transformers, Cryptography, Gradio, Starlette, Diffusers and Protobuf.
- Fix: source-controlled fail-closed quarantine via `HUNYUAN_RUNTIME_SECURITY_APPROVED=False` and exact quarantined digest metadata; `provider_runtime_configured("hunyuan3d")` now rejects the provider before submission, and Owner 3D status exposes the technical security gate. No VEX exception is used for the active venv findings.
- Security/tenant review: the audit used the local immutable image and vulnerability database only; no tenant content, provider credential, raw endpoint ID or generated artifact entered the evidence. No Production database mutation or paid GPU execution occurred.
