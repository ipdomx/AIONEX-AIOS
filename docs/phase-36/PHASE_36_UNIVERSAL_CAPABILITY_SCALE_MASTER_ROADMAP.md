...
> **2026-09-17 FR-06C5D3 source contract:** project execution admission, durable backup-worker cycle ownership and the five backup/restore API enqueue guards are specified in canonical `docs/project/PLAN.json` under `FR-06.host_state_cutover_admission`. API guards use the existing schema 2 authority inside enqueue transactions, including Owner transaction boundaries. A schema 2 row alone does not prove D3 deployment; the backup cycle snapshot still reports partial, unverified coverage. Other workers, producers, realtime admission and full-host drain remain follow-up work. Current tests, merge and deployment evidence remain exclusively in the canonical project hub.
