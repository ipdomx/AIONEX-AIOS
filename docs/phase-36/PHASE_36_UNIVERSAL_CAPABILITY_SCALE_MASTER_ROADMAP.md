...
- Checkpoint 4C1 proves OpenAI GPT Image 2 `inpaint` live on Production with a governed same-size alpha mask (`$0.014257`, output `772734` bytes, S3/DAG/Studio completion and cleanup; evidence SHA-256 `f981e250160ceeb5f0250050c3608304270859675779ce1f180346cb0c7fa630`). A single `background-remove` canary failed without recorded spend and was not retried; evidence SHA-256 `e3b09e6a4791775f7d8acccad2375ee95b399f833de6ea81dd1802a054ec911a`. Source hardening now removes GPT Image 2 background-remove/transparency claims, rejects transparent/background-remove contracts before durable row creation and before HTTP, while advancing trusted runtime evidence only for `generate/edit/inpaint`. Focused Factory `15/15`, provider adapter `11/11`, durable runtime `10/10`, Ruff/Mypy PASS; no migration. Stage 4 next moves to required design-family/editable-source exit evidence; 36E remains `in_progress`.
