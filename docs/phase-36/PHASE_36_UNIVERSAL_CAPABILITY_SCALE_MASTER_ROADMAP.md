...
- **Stage 3B reference transport merged/deployed disabled + image-to-video live acceptance PASS:** protected PR #452 merged as `2d632c75edaf427129e7974bb867c789520ac23b`; no migration was required and only the Video Provider Worker was rebuilt/recreated on `sha256:9a561b195bcfb9d048633f424a08e14d75baa405a7a81ab92288f8cacb2f479d`, still hard-disabled. Disabled-deploy evidence SHA-256 `a5f49da97524800ec529496a753fe2b55ff91bdfbd2e14584fed8d0369257728`. A no-spend precheck proved a real 1280x720 governed reference and exact `$0.40` route, then a separate one-shot `sora-2` image-to-video canary armed only the 4-second opening scene with `max_attempts=1`; exactly one submit completed after 11 polls, FFmpeg/ffprobe QA accepted a 2,004,303-byte MP4, actual cost was `$0.40`, and both input/output objects plus all synthetic DB rows were deleted/verified. Persistent worker remained disabled with cycles/errors `0/0`, global VideoExecution rows returned to `0`, `/ready` `10/10` PASS. Sanitized live evidence SHA-256 `bd3234b3e06a62082d474c46d209c6c267b894710a7d2c2b434357f480eb2f52`. Next: separate cost-capped `logo-to-video`; shared reference transport alone does not promote `reference-to-video`.
