...
- Next safe transition: baseline current production health/schema/counts -> fresh backup -> apply `0028` -> controlled backend/project-worker rebuild/recreate -> same-host replica activation -> live acceptance -> update this permanent record before any transition to 36C.
