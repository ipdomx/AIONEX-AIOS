...
- **Stage 3B reference transport candidate:** governed tenant-owned Media DAG references now feed Sora `input_reference` only after MediaObjectStore size/SHA-256 verification; cross-tenant references and corrupted storage fail pre-submit, and ambiguous reference submission remains fail-closed without blind resubmit. Local gates: Provider/Worker/Factory `27/27`, Pipeline/Authority/Media DAG `23/23`, Ruff/Mypy/compile/diff PASS. No provider request/spend or Production mutation; live image/logo/reference acceptance remains pending protected merge/deploy.
