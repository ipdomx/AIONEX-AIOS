...
### 36C budget-aware Production deploy + exact `$0.05` canary closeout — 2026-08-17T17:35:08Z

- Protected PR #414 `Phase 36C budget-aware paid routing` merged to `main` as `66bab574aef94544ef0e85d55fe2385d2116224a`. The first CodeQL Python/JavaScript attempts failed only because GitHub returned `503 No server is currently available`; both CodeQL jobs were rerun as attempt `2` and completed `2/2 SUCCESS`. Production Docker Build and every other protected gate completed green before merge.
- Pre-deploy safety gate: Production had `active=0`, `validated_provider_rows=4`, `plan_policy_records=2`, and `real_finance_records=0`. Fresh backup `/opt/AIOS/.deployment-backups/phase36c-budget-aware-deploy/aios-20260817T172306Z.tar.gz`, SHA-256 `834d87e9ba4da2c438f14e274209e2ae9daa1fc8a64c68018eee659b9451c7ac`; isolated PostgreSQL 16 restore smoke reproduced Alembic `20260817_0029`, one historical ProjectExecution, 15 providers, four validated-provider rows, two active plan-policy records, zero real finance records and zero active executions.
- Controlled deploy rebuilt/recreated only Backend and the two Project Workers. The merged runtime source is `66bab574`; all three containers contain the `remaining_budget`/`effective_policy` budget-aware code. Post-deploy settings remain deliberately unarmed: `PROJECT_EXECUTION_RUNNER_MODE=legacy`, `PROJECT_AI_LIVE_RUNTIME_ENABLED=false`, `PROJECT_AI_MODEL_REFRESH_ENABLED=false`, `PROJECT_EXECUTION_BUDGET_CAP_USD=0.05`, legacy/research model `gpt-5.6-luna`. PostgreSQL, Redis and Nginx were not recreated. Database stayed at Alembic `0029`; two Workers are online at capacity `2` each (aggregate `4`) with aggregate active `0`.
- Exact public-API budget proof is now closed with the real runtime shape, not an artificial token cap. A synthetic Paid ProjectExecution was created with the same `$0.05` budget the API uses, while the running legacy workers could not claim it. `DurableProjectAIRouteStore` persisted effective policy budget `$0.05` and primary estimate `$0.048022`. Real `ProjectAILiveProviderInvoker` execution completed planner/researcher/coder/reviewer `4/4` in four attempts, four scoped memories, four requests, zero retries/fallbacks and calculated cost `$0.0446096` (`durable_spent=$0.044609`, reserved `$0`). Routes were planner `gpt-5.6-sol`, researcher `gpt-5.6-terra`, coder `gpt-5.6-luna`, reviewer `gpt-5.6-luna`. All synthetic organization/execution data was removed in `finally` and post-cleanup counts are zero.
- Earlier 256/512-token diagnostics are retained only as truthful diagnostic history: they produced sanitized `provider_response` failures because an artificial output cap changes reasoning-model completion behavior. A direct coder diagnostic proved Luna succeeds at `512` tokens, and the final retained acceptance therefore used the unmodified Production provider default (`1024` maximum from the shared runtime) while the durable `$0.05` authority remained the spend guard. No provider config was permanently changed for the canary.
- Final exact-budget evidence file: `/opt/AIOS/.deployment-backups/phase36c-budget-aware-deploy/paid-exact-budget-real-runtime-20260817.json`, SHA-256 `bf571ecaae1d30bb72b1157e24a3138d0737e1fb12050751915664cf675b0d98`. Consolidated closeout: `/opt/AIOS/.deployment-backups/phase36c-budget-aware-deploy/final-budget-aware-closeout-20260817T173508Z.json`, SHA-256 `d9bd496c170a59ead4bb518109c9257c7a550d52449927f5ee7f61eb76bf532f`. Neither evidence file stores prompt or response bodies.
- Final health/safety snapshot: Backend/Workers/PostgreSQL/Redis/Nginx are healthy; active ProjectExecutions `0`; canary organizations/executions `0`; validated models remain OpenAI `gpt-5.6-sol/terra/luna`, Mistral `mistral-medium-3-5`, DeepSeek `deepseek-v4-pro`, Ollama `gemma3:4b`; two Owner plan-policy records remain present; real finance records remain `0`. Backend's actual container health endpoint `/ready` returned `20/20` HTTP 200, p50 `3.7ms`, p95 `5.9ms`, max `12.8ms`; the earlier public `/api/ready` probe was a non-existent Frontend route (`404`) and is not a Backend readiness signal. `vip-e.net` and `ai.vip-e.net/en/` return 200; `gabarot.vip-e.net` returns the expected Cloudflare Access 302. Backend/Worker logs show zero recent traceback/critical/panic/fatal matches.
- Budget-aware admission/routing is therefore **Production-verified** under the same `$0.05` execution ceiling used by the public API, closing the post-#413 budget blocker without raising user spend limits. Phase 36C remains `in_progress` because Paid user arming is still intentionally blocked on truthful Owner provider-finance configuration.
- **Remaining external Owner input is unchanged and now the sole Paid-arm blocker:** enter the real funded-credit baseline plus low/critical thresholds for each paid provider. Production currently has zero non-synthetic `project-ai-provider-finance` records. Until those account-owned values are supplied, keep runner `legacy`, live runtime `false`, and automatic model refresh `false`; do not fabricate finance values from usage history.
- Safe point: source/report/evidence are durable, Production is healthy and unarmed, active work is zero, rollback backup is verified, and no further live switch should occur without real Owner finance values.
