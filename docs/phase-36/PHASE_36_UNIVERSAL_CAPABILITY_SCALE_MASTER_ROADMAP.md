...
- Next safe transition: backup + migration `0028` gates are complete -> preserve current image IDs as rollback tags -> rebuild/recreate Backend and Project Worker only -> verify health/schema -> same-host replica activation -> live acceptance -> update this permanent record before any transition to 36C.
