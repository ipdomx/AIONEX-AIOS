...
### 36C live-provider acceptance + paid budget-aware routing checkpoint — 2026-08-17T17:01:46Z

- Protected deployment-evidence PR #412 merged to `main` as `7a6b31c3479f8dd4cbddf2432c6a41d97e65bc35` with every protected check green. Production remained healthy on Alembic `0029`, two Project Workers on `legacy`, Phase36C live runtime/model-refresh disabled, and active ProjectExecutions `0`.
- Owner-reviewed model evidence was then refreshed from the active provider credentials and committed only because all launch probes were complete: validated routes are `ollama:gemma3:4b`, `mistral:mistral-medium-3-5`, `deepseek:deepseek-v4-pro`, and OpenAI `gpt-5.6-sol` / `gpt-5.6-terra` / `gpt-5.6-luna`; `probe_failures=[]`, `unavailable=[]`, TTL `21600s`. Production persistence counts are Ollama `1`, Mistral `1`, DeepSeek `1`, OpenAI `3`.
- Owner launch access policy was committed without copying provider credentials: Free remains local-only `ollama:gemma3:4b`, zero project provider cost, offline/privacy mode and no fallback. Paid grants only the five fresh external launch routes (DeepSeek V4 Pro, Mistral Medium 3.5, GPT-5.6 Luna/Terra/Sol), with Owner policy ceiling `$1.00` and one approved fallback; the durable ProjectExecution cap remains the stricter `$0.05`.
- Bounded live-provider invoker acceptance completed `6/6 SUCCESS` through the production `ProjectAILiveProviderInvoker`, with no prompt/response body retained in evidence. Evidence file `/opt/AIOS/.deployment-backups/phase36c-live-provider-acceptance/acceptance-20260817.jsonl` has SHA-256 `8b4cfcdbf0b9bcef10647a1f1a0390887f9aa1cff10fa1c259cbae42295d5fb6`; total external acceptance spend was `$0.00041937`. Free Ollama acceptance was `$0`.
- Free full Project AI integration canary passed in the real Worker runtime (`NullPool` DB boundary): four roles (`planner/researcher/coder/reviewer`), four durable attempts, four project-scoped memories, `0` retries/fallbacks, `130` input + `128` output tokens, total provider cost `$0`. Canary tenant/execution data was removed after measurement. An earlier Backend-container canary correctly exposed that pooled asyncpg connections must not be reused across the runner thread/event-loop boundary; the real Worker containers are explicitly `DATABASE_POOLING_ENABLED=false`, and the same canary passed there with no source workaround.
- Paid planner diagnostics proved routing/evidence/coordination but also exposed a launch blocker. With an intentionally tiny `32` output-token acceptance cap, both GPT-5.6 Sol and Terra ended `provider_response`; at `128` tokens, Sol still ended `provider_response` but the approved Terra fallback succeeded (`1` retry, `$0.00158`, `22` input / `128` output tokens). These were canary output-limit effects, not auth/quota/billing failures.
- The real blocker appeared before provider execution when the complete four-role Paid plan was evaluated with the actual ProjectExecution budget `$0.05`: current `main` raised `ProjectAIBudgetExceeded: route plan exceeds durable execution budget`. Root cause: route ranking selected the highest-ranked model independently per task using the broader Owner Paid ceiling `$1.00`; only after planning did the durable layer clamp to the execution `$0.05`, so it rejected a plan that had cheaper validated alternatives capable of fitting the budget. Increasing the user budget was explicitly rejected as a workaround.
- Source correction is isolated on `fix/phase36c-budget-aware-routing`: the planner now filters ranked candidates against the **remaining** total budget and selects the highest-ranked affordable route; the durable route store computes an effective policy ceiling as `min(ProjectExecution.budget_cap_usd, Owner policy ceiling)` before planning/fingerprinting/persistence. The post-fix real launch-policy plan is `$0.0479034` total primary estimate: planner `gpt-5.6-sol`, researcher `gpt-5.6-terra`, coder `gpt-5.6-luna`, reviewer `gpt-5.6-luna`, with approved cheaper fallbacks.
- Regression evidence: routing + launch-model focused tests `13/13 PASS`; every `test_phase36c_*.py` test `61/61 PASS` on disposable PostgreSQL 16 + Redis 7 at Alembic `0029`; focused Ruff PASS; focused Mypy PASS; `git diff --check` PASS. Disposable services were removed. Production source/runtime has not been changed by this correction yet and remains `legacy` + live-runtime disabled.
- Provider finance boundary remains truthful: no `project-ai-provider-finance` Owner records currently exist, because actual funded balances and low/critical thresholds are Owner/account facts and were not invented. Periodic credit-alert and immediate billing/quota escalation code remains covered by the green Phase36C suite, but final Paid live activation must not claim configured balance monitoring until real funded values are supplied/recorded.
- Safe next action: protect and merge the budget-aware routing correction, deploy only after protected CI is green, rerun the real `$0.05` Paid full-plan/canary and two-Worker route/fallback/memory acceptance, then stop at any remaining real-finance/external-provider gate before arming user traffic.
