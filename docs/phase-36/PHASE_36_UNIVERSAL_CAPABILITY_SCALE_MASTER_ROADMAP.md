...
- **Stage 2 source/test candidate:** Alembic `20260819_0034` adds tenant-scoped parent `video_executions` and per-scene `video_scene_executions`. Explicit arm-before-spend, idempotency, sequential continuity claims, durable `submitting` markers, reconcile-without-resubmit after ambiguous worker crashes, poll-only recovery after provider job persistence, bounded definitive resubmit, dead-letter budgets, lease/fencing, sanitized metadata, truthful cost aggregation, malformed-MP4 rejection, failed-scene-only resume and assembly-gated finalization are implemented on the existing Phase36D Media DAG. Migration `0034 -> 0033 -> 0034` PASS; authority/recovery `10/10`, combined Video/Studio/36D/Alembic regression `27/27`, registry/VideoFactory `19/19`, Ruff/Mypy/compile/diff PASS. `video-continuity-resume` is only `source_built`; no 36F capability is runtime-verified yet.
