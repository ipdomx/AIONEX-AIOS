...
- Next safe transition: backup + migration + rollback tags + Backend/Worker1 recreate gates are complete -> scale same-host Project Worker to 2 replicas -> require both healthy and registry capacity 4 -> live Backend/ingress/fabric acceptance -> update this permanent record before any transition to 36C.
