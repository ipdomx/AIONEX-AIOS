...
- Next safe transition: backup/restore gate is complete -> apply `0028` -> verify schema/data -> controlled backend/project-worker rebuild/recreate -> same-host replica activation -> live acceptance -> update this permanent record before any transition to 36C.
