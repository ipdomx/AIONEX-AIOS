...
## 2026-09-08 — G20 Cloudflare R2 off-site DR closed in Production

The Owner supplied a private Cloudflare R2 Standard bucket and bucket-scoped Object Read & Write Account API authority. Protected PRs #592/#593/#594 completed the durable R2 replication/restore implementation and final root-only secret bootstrap. The canonical credential is now root:root mode 0400 outside Git; the Backup Worker receives a read-only root source and the entrypoint creates only a private UID/GID 1000 mode-0400 runtime copy before privilege drop. Transitional workspace and `/etc` credential copies are absent.

Fresh post-merge Production backup `1b9a0849-8163-4211-b6f2-f8dd0832c4ee` completed at 20,670,294 bytes with SHA-256 `ff9fdf27b9d40088ac1e801546d73bc6e6156e6c3c6c490753ef9404db05885c` and `offsite_status=completed`. Its R2 prefix contains the required database dump, manifest and 3D snapshot. Remote restore validation `6fe89481-13e6-4cf4-b1d6-ddb7d72a78ce` completed with `validated=true`, `offsite_required=true`, `offsite_validated=true`, `three_d_snapshot_required=true` and `three_d_snapshot_validated=true`. Finalization is 100%, all ten health/release checks pass, Phase 36 current batch remains COMPLETE, Production is 35/35 running with zero unhealthy and zero current restarts, and `/ready=200`.

**Reconciliation update:** G20 moves from external authority/infrastructure open to **closed internally/runtime with real external storage authority supplied**. Multi-host HA (G18) and host/block-device encryption (G21) remain separate infrastructure boundaries and are not implied by R2 DR. AWS-backed storage remains neither required nor relabeled; AWS/Bedrock/XR/payment activation remains the Owner-deferred tail.
## 2026-09-08 — Owner deferral update: G21 hosting disk encryption

The Owner explicitly deferred G21 host/block-device encryption while awaiting written technical evidence from Namecheap. No reboot, reinstall, reprovisioning or storage mutation is authorized while that response is pending. G21 joins the Owner-deferred tail and is not a launch blocker for the current application/runtime certification; no encryption-at-rest claim is made without hosting evidence.

G16 was re-evaluated after this deferral. The repository contains the official LiveKit CLI load-test capability and the live SFU/TURN/Egress stack is healthy, but a same-host load generator cannot truthfully certify 1000 Internet media clients because it shares the same failure domain/network path. The existing 1000-client PostgreSQL/Redis/control-plane acceptance remains valid; final 1000-concurrent audio/video capacity remains an external off-host load-generation proof and is not fabricated from an on-host stress test.
