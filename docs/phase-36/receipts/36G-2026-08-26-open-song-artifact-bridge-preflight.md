...
## Final GPU image hardening gate

The first complete AIONEX Open Song image built from merge `15c69907e413c8cb2291f63dc9908a36cfacc7f5` passed all 25 Docker build steps and produced a non-root linux/amd64 candidate, but a full Trivy 0.72.0 HIGH/CRITICAL scan correctly blocked publication: 346 HIGH and 24 CRITICAL findings were inherited primarily from build-only Ubuntu headers in the upstream ACE-Step image. No image was pushed and no RunPod endpoint or GPU job was created.

A disposable hardening proof then upgraded the available OpenSSL/GnuPG security fixes, purged development-only packages including `linux-libc-dev`, upgraded compatible runtime libraries (`diffusers 0.38.0`, `orjson 3.11.6`, `Pillow 12.3.0`, `python-multipart 0.0.30`, `Starlette 1.3.1`), and removed `setuptools`, `wheel`, `uv`, and `uvx` from the final runtime. ACE-Step API, Demucs, RunPod, FastAPI, Transformers, and the AIONEX handler import checks all passed. A second full Trivy scan reduced the gate to 12 HIGH and 0 CRITICAL findings.

The remaining 12 findings are handled only by package-and-version-scoped OpenVEX statements, never wildcard or image-wide suppression. Six are Trivy package-metadata matches for `CVE-2022-42919` against Python 3.11.15 even though the NVD affected ranges stop before Python 3.11. The other six are unreachable in this serverless contract: Gradio UI/Space/cookie paths are not launched; Transformers remote-repository/trust-remote-code controls are absent because the runtime is offline with exact baked model revisions; and Flash Attention cannot receive an adversary-controlled checkpoint path. Trivy with that exact 12-statement VEX returned exit 0 on the disposable hardened candidate. Final publication still requires the same hardening to pass protected CI, a rebuild from the resulting merge SHA, a fresh SBOM, and a fresh Trivy+VEX scan of that final image.
