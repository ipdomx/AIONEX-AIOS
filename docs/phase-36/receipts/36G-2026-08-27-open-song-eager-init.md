...
## Live acceptance v8 — Full Song runtime closure

- Final candidate `aionex-open-song-f8259d5` reached Ready/idle with zero provider jobs before acceptance. Immutable runtime image: `sha256:6b6ce10bda3adc378fff230b307ac1ce9f86aaf21d82cd6e1f9c9b9f2a19ea34`; handler SHA-256 `15f8b34e8f45ce3f156cd2d0e00df532acd3803e5bdff4370ab670e634652a37`; package-equivalent CycloneDX SHA-256 `ea9d47313f92ed7af3eb643182b372c18d1d2eea8291af8f63f15e9c30395f11`.
- The Cloudflare Error 1010 reproduction was fixed without relaxing Cloudflare policy: the handler now sends the explicit bounded User-Agent `AIONEX-AIOS/OpenSong-ArtifactBridge/1.0`. Exact urllib Artifact Bridge smoke changed from HTTP 403 / Error 1010 to PUT `201` and DELETE `204` before the provider acceptance.
- Acceptance v8 executed exactly one real RunPod submission. Terminal result: `status=pass`, `attempts=1`, `retried=0`, provider state `completed`, billed `76.0` seconds, actual cost `$0.02584`, four stems returned. RunPod health after the acceptance recorded `completed=1`, `failed=0`, `inProgress=0`, `inQueue=0`, `retried=0`, with no control-plane errors.
- The complete durable graph passed: Full Song, `vocals`, `drums`, `bass`, `other`, local mix, master, final export and waveform were all `completed`. Final output is a 30.0-second WAV, `5,760,078` bytes, checksum `d27b67252edd1e6239a32a5d1ea5af491f62a02c1fac65951631f73dc1e7b77d`.
- Final audio QA PASS: integrated loudness `-14.03 LUFS`, loudness range `5.4 LU`, true peak `-1.1 dBTP`, sample peak `-1.427 dB`, `clipped=false`; all integrated-loudness, LRA, true-peak and clipping checks passed.
- Studio materialization PASS: `studio_revision=2`, current Studio asset revision `2`. Public evidence returned no raw provider job ID, endpoint ID, credential or storage locator.
- Cleanup PASS: Organization, AudioSongExecution and MediaGraph residue all returned to `0`. Persistent acceptance evidence SHA-256 is `ad01ab9ed9c694b5ac9f5dfa5b5caec968b60b6a16300d8c64ae1cd985f632c5`; live balance evidence SHA-256 is `21fe6cbe9d36e82550872e58e993e0841337fb583a8e6d53ed4dd28e69ec3d3a`.
- Production `audio-song-worker` remained Healthy and `AUDIO_SONG_LIVE_ENABLED=false`; the temporary acceptance-only runtime secret copy was removed after evidence persistence. No automatic retry, resubmit or cross-provider fallback occurred.
- Runtime conclusion: `ace-step-open-song-runtime-acceptance` is satisfied and removed from `song-production`. Capability maturity advances from `source_built` to `runtime_verified`. The separate `music-rights-and-ai-generated-disclosure` gate remains intact and is not represented as an internal implementation failure.
- The secondary RunPod account/failover remains outside this chapter closure and is not activated or modified here.
