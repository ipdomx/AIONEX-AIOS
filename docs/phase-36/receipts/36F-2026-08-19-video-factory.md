...
- The durable record separates **provider submission attempts** from **provider polling count**. Before a provider job ID exists, a claim consumes `attempts`; after `provider_job_id` is durably recorded, later claims consume only `poll_count`. This allows a worker to crash/restart during a long async provider job without treating each poll/recovery as a new submission attempt.
- Provider job identity is immutable after persistence and `(provider, provider_job_id)` is unique. Lease ownership, expiry and fencing reject stale workers after reclaim. Bounded submission/poll exhaustion dead-letters the execution instead of looping indefinitely.
...
- Migration verification on disposable PostgreSQL 16: fresh upgrade to `0034` PASS; `0034 -> 0033 -> 0034` recovery PASS. Video/Alembic gate `10 PASS`; combined Video + Phase36D Media + retained DesignImage/fencing + Alembic regression `18/18 PASS`. Ruff PASS; focused Mypy PASS; Python compile and `git diff --check` PASS. Disposable PostgreSQL/Redis/network resources were removed.
- Stage 2A contains **no provider HTTP adapter, no provider video call/spend, no Production DB migration and no worker activation**. The unresolved submission edge — provider accepts a job but the response is lost before its job ID is recorded — must be handled in Stage 2B through provider-supported idempotency/request identity where available; the authority will not pretend that network ambiguity is solved by local retries alone.

### Next safe gate

Protect and merge Stage 2A, then deploy Alembic `0034` with a fresh Production backup/restore and keep video execution unarmed. Only after the durable authority is deployed should Stage 2B add exact Sora/Gemini adapters, a live-disabled Video Provider Worker, governed VideoPlan -> Media DAG pipeline creation and image/logo/reference inputs. Before any paid live canary, prove provider submission idempotency/recovery semantics and exact MP4/FFprobe QA.
