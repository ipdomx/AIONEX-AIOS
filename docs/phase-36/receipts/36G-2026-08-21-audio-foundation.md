...
## Stage 8 — open-weight complete-song source candidate

Stage 8 adds a provider-neutral full-song product contract and two explicit ACE-Step runtime routes without claiming that source evidence is live audio evidence. The default paid-capable route is a hard-disabled RunPod Serverless boundary; the official ACE-Step Hugging Face Space is acceptance-only and cannot become a Production user route. Both permit exactly one submission attempt, forbid automatic retry/resubmit/cross-provider fallback, and require original/licensed/public-domain lyrics, commercial authorization, Provider terms and AI-generated/synthetic-vocal disclosure. Named-person/artist imitation, known-person voice, voice transformation and cloning remain rejected.

The supply chain is pinned to ACE-Step source `dce621408bee8c31b4fcf4811682eb9359e1bc94`, official linux/amd64 base image `sha256:c289cb5c0cbc60d428baa9283a49966d2fe54ecf2028fa254f99f164f3953159`, base-model revision `e432212fec32b8965a14ffa57ae653438d6abd14`, official Space/Turbo model revision `19671f406d603126926c1b7e2adc169acbcade22`, 4B language-model revision `0a3ec94b557aea7d508da38b31cfe7341f6ff737`, Demucs source `ef66d254cd6d558e207eeff2c4b8d053db2e77dd`, and `htdemucs` checkpoint SHA-256 `8726e21a993978c7ba086d3872e7608d7d5bfca646ca4aca459ffda844faa8b4`. The audited ACE-Step image is only the base; the RunPod route cannot arm until a separate AIONEX handler image has its own immutable image/index digests, SBOM hash, handler-source hash and hashed Endpoint binding.

Alembic `20260823_0039` introduces the separate `audio_song_executions` authority rather than overloading fixed-price Lyria/Stability rows. It persists tenant scope, hash-only private text evidence, rights/disclosure, exact model/source/image bindings, one-attempt lease/fencing, durable provider Job identity, bounded polling, GPU-second cost evidence, full-song/stem manifests and final Studio/QA evidence. The `audio-song-worker` is profile-gated and hard-disabled by default. It claims only `runpod-flex-a40`, persists `submitting` before the network boundary, records the Provider Job ID before polling, resumes only that Job, restricts artifact downloads to an explicit HTTPS host allowlist, validates 48 kHz stereo PCM WAV bytes and atomically opens the existing FFmpeg Media DAG. Expired pre-ID submission becomes `needs_review`; it is never resubmitted automatically.

The isolated RunPod handler starts ACE-Step only on authenticated localhost, runs offline against baked model revisions, produces one synthetic-vocal song, separates `vocals/drums/bass/other` through the pinned Demucs checkpoint, canonicalizes every output to 48 kHz stereo PCM WAV, uploads private AES-256 S3 objects, and returns short-lived HTTPS locators only from an allowlist. The Backend remains authoritative for approval, cost, tenant scope, local mix/master/waveform/export, Studio revision and cleanup.

### Stage 8 source verification

- The final security-corrected immutable staged-tree snapshot (`6f7a0fefc2ab39bf76610f36c894fc599805e7f4`) passed root open-song/Audio/Phase36 contracts `78/78`; retained log SHA-256 `caea46fd48e77d6d77cf7f891c59a0d7c6c7ddaba932c109ea7043ecfbbacac6`.
- Focused Backend/Compose/authority/provider/worker contracts passed `60`, with one CI-only live-backup smoke skipped; retained log SHA-256 `8f0d981a912274c19ee76ff47db0ece8e00a0cd79389e27c763093ab4177bc95`.
- The same immutable snapshot passed complete Backend `979`, with one CI-only test skipped and no failure; retained authoritative log SHA-256 `fd96190b46da82b559c60776b82716f48f1a76971b0a9d6fc3de40631d994936`.
- Alembic fresh upgrade and `0039 -> 0038 -> 0039` recovery passed with `audio_song_executions=0`, all Song/Media/Studio active counts `0`, PostgreSQL/Redis critical hits `0/0`, and every disposable container/network removed. Ruff, split Mypy for Core/Backend/isolated GPU package, Python AST across `23` files, explicit health-file Secret Sink scan, `git diff --check`, and both Production Compose renders passed. Consolidated security-corrected frozen gate SHA-256: `425d8f72da6f6316cb2d696b7e6334e7b51d71c7e2bbc2fa17a635b9f7a0b059`.
- A real FFmpeg 9 acceptance used synthetic provider-boundary WAV inputs and the durable authority/Media Worker. Four render steps completed (`mix/master/waveform/export`), final WAV was `10.0s`, `1,920,078` bytes, `36G.audio-qa.v1` passed at `-14.3 LUFS / -5.8 dBTP`, Studio reached revision `2`, and all synthetic rows/object directories were removed. Provider generation requests/GPU Jobs/spend were `0 / 0 / $0.00`. Evidence SHA-256 `d46002c4d45178b6c73e95ca4a09d9af6d877081bcb0dcaae72eb1108e7d03f8`.

### Official Space acceptance stops

Three separately invoked official ACE-Step Space submissions crossed the generation boundary, each with `batch_size=1`, no automatic retry/resubmit/fallback and reported Provider spend `$0.00`:

- the explicit supported Turbo request used Space revision `7403460e9b34972f760317b56048f6cb9d4a3a11`, Turbo model revision `19671f406d603126926c1b7e2adc169acbcade22` and 4B language-model revision `0a3ec94b557aea7d508da38b31cfe7341f6ff737`; it ended `GPU task aborted` and returned no audio. Sanitized attempt/result SHA-256 values are `f254bb4ea55f0b683e5f63bfd65f9f6a3d1d599cda6720ef73264d7d87dc6945` and `77c647cc42e6647bc38feb4426f2267bdd73e0f1621360cc5bba596e3e1daec0`;
- a separate unauthenticated XL-Turbo request stopped immediately because the shared ZeroGPU quota was exhausted (`180s requested` with `0s` remaining); it returned no audio;
- a separate authenticated XL-Turbo request was manually invoked after that quota stop, ran until `GPU task aborted`, and also returned no audio.

The two XL attempt records named revision `d4a0dc2d788a1f78c038db8a50dc69fd24d57985`, which does not resolve. A later read-only model audit verified current official XL repository revision `d4a0b288b83ebb7e25a8c0b32c573c22e134e8ee`; that inventory correction does not retroactively validate either failed generation. The combined XL closeout proves `2` XL requests, `0` successful generations, `0` audio outputs and `$0.00` spend; its sanitized evidence SHA-256 is `815ea4ef8bf10291f330ef7d925a3b205ccdd3ebfc538ecc757493efb2d70ad0`. All three Space attempts remain failed/needs-review evidence, not runtime acceptance.

## Stage 8 maturity decision

`song-production` advances only from `specified` to `source_built`. The source and isolated FFmpeg exit path now separate evidence for lyrics, composition/synthetic vocals, four stems, mix, master, waveform and final export, but no real ACE-Step output passed the complete runtime acceptance. The maturity gate is therefore the route-neutral `ace-step-open-song-runtime-acceptance`, plus music-rights/AI-disclosure evidence. Phase 36G remains `in_progress`; the permanent Worker remains disabled and Production/Alembic remain unchanged by this source batch.

## P36-0093 through P36-0099 — concurrent harness, provider attempts, source drift and security findings were closed truthfully

- **P36-0093:** a concurrent first full-Backend wrapper escaped its shell return-code variable and stopped before pytest. Its result is invalid and not counted.
- **P36-0094:** disposable PostgreSQL/Redis were removed before a second concurrent full-Backend container was discovered. That test lost its isolated dependencies, was stopped and retained only as invalid incident evidence.
- **P36-0095:** a later live-mounted full-Backend run reported `978 passed, 1 skipped`, but three Stage 8 files changed while it was running. Its otherwise-green result is excluded because the source was not immutable. A first immutable snapshot `1a364b2b4e046860c96ad9c55309a10aa5e71556` closed that harness issue, and the later CodeQL remediation was reverified on final immutable snapshot `6f7a0fefc2ab39bf76610f36c894fc599805e7f4`.
- **P36-0096:** the concurrent edit proposed XL revision `d4a0dc2d788a1f78c038db8a50dc69fd24d57985`, which does not resolve. Read-only verification found current official XL revision `d4a0b288b83ebb7e25a8c0b32c573c22e134e8ee`; the invalid source drift was rejected rather than silently changing the reviewed Turbo acceptance route. Two separately invoked XL diagnostics did cross the Space boundary, but both failed without audio and therefore do not advance maturity. Incident evidence SHA-256 values: P36-0095 `77e9cfac8a49ceecbf2b32966bf491c45e628d27e180ac4daeaf3aec91d173d7`, P36-0096 `5320316aea4c659db0f188c1c64c79f907da483818283bc064a9cf5f77e8c92a`.
- **P36-0097:** GitHub Advanced Security correctly rejected writing a health JSON payload built from an object that also holds the RunPod credential. Although the prior `public_snapshot()` omitted the credential, the health sink no longer receives any Runtime Binding object or derived values: it persists only `runtime_binding_loaded` and `runtime_binding_returned=false`. A new regression asserts the API key, Endpoint ID/hash, image digests, SBOM and handler-source hash are all absent from the health file. Focused remediation passed `27/27`, log SHA-256 `ff3655d5514ec033e206052170f8e43f7f596e34ad82858430a768a4d7a3cde6`; final frozen Backend passed `979`, and security gate SHA-256 is `425d8f72da6f6316cb2d696b7e6334e7b51d71c7e2bbc2fa17a635b9f7a0b059`.
- **P36-0098:** two additional concurrent XL Space attempts were discovered after the source PR merged. The unauthenticated request was quota-blocked and the authenticated request ended `GPU task aborted`; both returned no audio, reported `$0.00`, and had no retry/resubmit/fallback. The mounted Hugging Face token was hashed for identity, verified absent from all retained evidence, then securely deleted; both exited containers were removed. Closeout evidence SHA-256: `815ea4ef8bf10291f330ef7d925a3b205ccdd3ebfc538ecc757493efb2d70ad0`.
- **P36-0099:** an unreviewed local replacement of `audio_open_song_pipeline.py` appeared outside PR #484 and was never committed, pushed, tested by CI or deployed. The reviewed merged source was restored and the follow-up branch started clean from `origin/main`. The retained diff capture raced a concurrent restore and is intentionally recorded as zero bytes rather than inventing missing evidence; incident record SHA-256 `deaba2ec1cce2dfaa453e840673fc757f2970827d1a02562ac831c69ee416645`.
- Production and user data remained untouched throughout. Harness/security cleanup added `0` Provider requests/GPU Jobs/spend; the separately classified XL diagnostics added exactly `2` Space requests, `0` successful generations, `0` output artifacts and `$0.00` reported spend.

## Stage 8 source merge closeout

Protected PR #484 merged as `6396aa3018bb3547be5cd46d76edbf37b97d4374` on `2026-08-23T20:36:07Z` after all `13` reported checks passed, including Backend Tests, Production Docker Build, CodeQL Python/JavaScript plus aggregate CodeQL, SBOM/vulnerability, dependency security, repository hygiene, browser boundaries, Phase 36 reporting and Compose/DNS validation. The merge changed source only: Production remained at source `d2dadeb2c6fc59762935bc0b0c6dffe1cb136ceb`, Alembic `20260823_0038`, with no `audio-song-worker`, no `0039` migration, no RunPod Endpoint activation and no service restart.

## Stage 8 next safe gate

Perform only a separate hard-disabled Backend/Migration/Worker activation from merged source with a fresh Production backup, disposable restore proof, exact image identities, zero active Song/Media queues and zero GPU submission. A later explicit acceptance may use the official Space only when quota is available or a separately built/SBOM-verified/funded RunPod handler Endpoint; never invoke routes automatically and never raise `song-production` above `source_built` until real full-song, four-stem, local QA, Studio and cleanup evidence all pass.
