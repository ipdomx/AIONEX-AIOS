...
### Official Space one-attempt acceptance stop

One separately authorized request was sent through the official ACE-Step Space using pinned Space revision `7403460e9b34972f760317b56048f6cb9d4a3a11`, the explicitly selected supported Turbo model revision `19671f406d603126926c1b7e2adc169acbcade22`, and 4B language-model revision `0a3ec94b557aea7d508da38b31cfe7341f6ff737`. The Space returned `GPU task aborted`; no audio was returned, Provider spend was `$0.00`, and automatic retry/resubmit/fallback remained false. The attempt is retained as `needs_review`, not acceptance. Sanitized attempt/result SHA-256 values are `f254bb4ea55f0b683e5f63bfd65f9f6a3d1d599cda6720ef73264d7d87dc6945` and `77c647cc42e6647bc38feb4426f2267bdd73e0f1621360cc5bba596e3e1daec0`. The deployed Gradio schema also advertises `acestep-v15-xl-turbo` and currently defaults to it, but only a read-only MIT/public model audit was performed for current XL revision `d4a0b288b83ebb7e25a8c0b32c573c22e134e8ee`; no XL generation or runtime claim is made.
...
## P36-0093 through P36-0097 — concurrent harness, source drift and security finding were closed truthfully
...
- **P36-0096:** the concurrent edit proposed XL revision `d4a0dc2d788a1f78c038db8a50dc69fd24d57985`, which does not resolve. Read-only verification found current official XL revision `d4a0b288b83ebb7e25a8c0b32c573c22e134e8ee`; because the only real Space attempt explicitly used Turbo and XL produced no audio evidence, the invalid drift was rejected rather than silently changing the route. Incident evidence SHA-256 values: P36-0095 `77e9cfac8a49ceecbf2b32966bf491c45e628d27e180ac4daeaf3aec91d173d7`, P36-0096 `5320316aea4c659db0f188c1c64c79f907da483818283bc064a9cf5f77e8c92a`.
...
- Production, Provider state and user data were untouched throughout; additional Provider requests/GPU Jobs/spend caused by the harness/security corrections were `0 / 0 / $0.00`.

## Stage 8 next safe gate

Protect and merge the source candidate through CI first. Then perform only a hard-disabled Backend/Migration/Worker activation with fresh backup/restore evidence and zero GPU submission. A later explicit acceptance may use either the official Space when its GPU quota is available or a separately built/SBOM-verified/funded RunPod handler Endpoint; never invoke both automatically and never raise `song-production` above `source_built` until real full-song, four-stem, local QA, Studio and cleanup evidence all pass.
