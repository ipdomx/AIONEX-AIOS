...
Status: **BLOCKED_ON_EXPLICIT_LIVE_LEGAL_BUDGET_STOPLOSS_AND_LAUNCH_APPROVAL**
...
### GS-12 no-spend approval separation production closeout — 2026-08-15
- PR #360 passed every required GitHub gate and merged to `main` as `6f153636a70a406207943d3cce46d6d99cd8e345`. Successful gates included Full Backend Tests, Core Owner / Release / Web Contracts, Owner/VIP browser boundaries, Owner/VIP frontend quality, Policy/Resilience, CodeQL, Dependency Security, repository hygiene, SBOM/vulnerability checks, and Production Docker Build with backup/restore smoke.
- Production `main` was fast-forwarded to the merge commit. Backend and Operations Observer were rebuilt/recreated from the same new backend image because both execute GS-12 controlled-pilot readiness/runtime logic. Frontend/Nginx/schema were unchanged. Backend and Observer returned healthy on the same image digest; Alembic remains `20260815_0026 (head)`.
- Post-deploy live readiness for the selected `Sell Save` pilot is now intentionally split and verified: `provider_gate=true`, `execution_adapter_gate=true`, `legal_gate=false`, `budget_gate=false`, `stop_loss_gate=false`, `launch_gate=false`, `live_provider_mutation_allowed=false`, and `real_spend_allowed=false`. Currency remains provider-derived `EUR`; all monetary limits remain unset.
- The historical no-spend approval/evidence is preserved separately from live legal consent. It remains `approved=true`, scope `single-paused-campaign-create-read-delete`, `completed=true`, with `real_spend_minor=0`. Because the successful validation predates the new explicit `consumed` field, that legacy record does not contain every newer metadata field; however the deployed validator treats `completed=true` as consumed/non-reusable. A production preflight proved reuse is rejected as `meta-owned-write-no-spend-approval-consumed` **before token read or provider call**.
- The new dedicated no-spend approval path is private-only: unauthenticated private POST reaches Owner auth (HTTP 401), while the same public-origin POST remains HTTP 404. The route records approval only; it does not execute Meta, does not change the live legal gate, and does not enable spend.
- Future no-spend validations are single-use by construction: approval consumption is committed durably before credential/provider access; a failed attempt cannot reuse the same approval; successful completion is recorded on the same structured approval. This behavior passed focused, Growth/Social, Full Backend and Core suites before merge (`31`, `101`, `598`, and `680` passes respectively).
- No provider mutation, budget change, launch authorization or real advertising spend was executed during this deployment/acceptance. The only real owned-account mutation in GS-12 remains the already completed one-time PAUSED Campaign validation, deleted immediately with spend zero.
- Provider identity/write proof and the guarded execution adapter are complete. GS-12 is now blocked only on **new explicit live-advertising inputs**: (1) a separate legal/policy acknowledgement for launching paid advertising on `Sell Save`, (2) explicit maximum total budget, maximum daily budget, maximum CPA and minimum ROAS stop-loss values in EUR, and (3) a separate launch authorization after every readiness gate except launch is green. The prior no-spend approval must never satisfy item (1).
- Safe restart point: do not repeat provider/token/no-spend/adapter work. Re-read this roadmap, verify the live legal gate is false and real spend remains disabled, then continue only from the first newly supplied live-advertising consent/control value.
