...
### GS-12 runtime hardening production closeout — 2026-08-15
- PR #344 passed every required GitHub production gate and merged to `main` as `405370ec81ff0a79784c555d777c20ede1597fe6`.
- Production pre-migration safety check confirmed exactly three controlled pilots, all read-only, with `real_spend_enabled=0` and `live_mutation_enabled=0`; therefore the single-live-scope invariant could be applied without conflicting production rows.
- Backend production image was rebuilt from the merge commit using the existing Meta sandbox/owned-readonly Compose overrides. Alembic `20260815_0026` was applied in a one-off Backend container before any live service replacement.
- Backend and Operations Observer alone were force-recreated after the migration. Both returned healthy; production Alembic reports `20260815_0026 (head)`.
- PostgreSQL confirms the unique partial index `uq_growth_controlled_pilots_live_scope` exists. Post-deploy state remains three read-only pilots (Meta owned assets, Meta sandbox, Telegram owner bots), all with no launch authorization, `live_provider_mutation_allowed=false`, and `real_spend_allowed=false`.
- A rollback-only production database proof exercised the new runtime guard with no provider API call: a temporary synthetic live-spend pilot was armed only inside one uncommitted DB transaction using temporary synthetic capability evidence; initial `runtime_authorization` passed, then forced expiry caused immediate auto-disarm, and a second arm followed by provider-capability revocation was auto-disarmed by `reconcile_runtime_pilots()`.
- The rollback-only proof produced two in-transaction `growth.pilot.runtime_auto_disarmed` audit events, executed no provider call and no spend, then rolled back. Independent verification afterward found `synthetic_organizations_remaining=0`, `synthetic_pilots_remaining=0`, and durable `meta/ads.manage` restored to its genuine `sandbox_write_verified` state.
- Runtime safety is now layered: per-action current-state authorization, 30-second independently committed observer reconciliation, per-scope PostgreSQL advisory locking, and a DB unique partial index preventing simultaneous spend authorization on the same managed ad account.
- No real Meta owned-account write, budget edit, audience upload, publish/send, automatic execution, or real advertising spend occurred during this deployment or proof.
- All safe no-spend GS-12 work currently possible with installed credentials is complete. Remaining live-spend gates are external/owner inputs: target AIOS tenant + managed ad-account opaque reference, owned/live Meta credential with `ads_management` and provider/app approval, no-spend live-write adapter verification on that authorized target, legal/policy reference, explicit currency + total/daily budget + CPA/ROAS stop-loss values, then separate per-pilot launch authorization.
- Next safe development work that does not require those inputs: prepare and CI-test the live-owned Meta write validator in fail-closed mode only; it must remain non-executable until target/credential confirmation is present and must never infer budget or launch authorization.
