...
Status: **IN_PROGRESS_USER_CAMPAIGN_ADVISOR_MERGED_LOCAL_ACCEPTANCE_PASS_EXTERNAL_AI_PORTAL_ROUTING_BLOCKED_CLOUDFLARE_TOKEN_REQUIRED_LIVE_LEGAL_AND_LAUNCH_GATES_REMAIN**
...
### GS-12 user campaign advisor production deployment and external portal-routing checkpoint — 2026-08-15
- PR #363 (`feat: add user AIOS paid campaign advisor`) passed every GitHub gate, including Core, Backend, Owner/VIP browser boundaries, VIP static build, CodeQL, SBOM, dependency/secret checks and Production Docker backup/restore smoke, then merged to `main` as `9b02683a976178405a7d9fe734bca9cbe984cf90`.
- Production `main` was fast-forwarded to the merge. Backend and the `portal` service were rebuilt/recreated from that commit using the existing production Compose stack; both returned healthy and Alembic remains `20260815_0026 (head)`.
- User Campaign Advisor is now implemented end-to-end in the repository: six localized `/campaigns` pages, authenticated user API, atomic Campaign + Ad Set + Creative + Ad preparation, synthetic pre-launch AIOS analysis, unchanged user-selected budget, explicit `guaranteed_results=false`, no user approval route, and Super Owner-only approval queue.
- Local/CI-equivalent evidence on the final branch: focused campaign/Owner contracts `26 passed`; Backend static Black/Ruff/Mypy PASS across `175` source files; VIP integrity + TypeScript + lint + six-locale static build PASS (`115` pages) and static smoke PASS (`94` URLs); Full Backend `604 passed, 1 skipped, 0 failed`; Core `680 passed`; roadmap/ingress `14 passed`.
- Production rollback-only acceptance created a synthetic organization/user and an Owner override inside one transaction, exercised the atomic Campaign Advisor with user-selected EUR budget, confirmed advisory-only behavior, unchanged total/daily budget, `guaranteed_results=false`, user self-approval rejection, `provider_call=false`, `real_spend=false`, then rolled back completely. Post-check found `0` synthetic organizations and `0` synthetic campaigns.
- Production database safe-state after cleanup: `0` real paid campaigns, `0` pending-owner campaigns, `0` spend-enabled pilots. The existing Sell Save controlled pilot remains fail-closed with pilot-only controls EUR 20 total / EUR 5 daily / max CPA EUR 3 / min ROAS 1.5; `legal=false`, `launch=false`, `live_provider_mutation_allowed=false`, `real_spend_allowed=false`. Meta capability remains `live_write_verified` with provider `spend_allowed=false`.
- Public API routing is correct externally: unauthenticated `https://api.vip-e.net/api/v1/growth-social/paid-campaigns` returns application auth (`401`), while the Owner paid-campaign API is not exposed on the public channel (`404`).
- External User Portal acceptance exposed a separate deployment-routing blocker: the new portal container serves `/en/campaigns` and `/ar/campaigns` locally, but `https://ai.vip-e.net/en/campaigns/` still returns `404` and external portal responses carry an older `Last-Modified` timestamp from 2026-08-11. Current cloudflared remote ingress contains only `api.vip-e.net -> nginx:8080` and `gabarot.vip-e.net -> nginx:8081`; `ai.vip-e.net` is absent, so external requests never reach the current `portal` origin at `nginx:8082`.
- The repository already contains a fixed-purpose, fail-closed activation operator `scripts/phase29b/activate_portal_hostname.py`. It can only preserve API/Owner routes, add/correct `ai.vip-e.net -> nginx:8082`, maintain the mandatory `http_status:404` catch-all, update the single proxied CNAME to the existing production tunnel, verify post-state, and roll back tunnel configuration if DNS mutation fails.
- The required account-scoped Cloudflare credential is currently absent; the connector-only production tunnel token cannot modify DNS/remote ingress. Therefore **no Cloudflare DNS or tunnel mutation has been performed**.
- Prepared root-only helper `/opt/aionex-ops/install-phase29b-cloudflare-token.py` outside Git. It accepts only a hidden interactive Cloudflare API token, derives the exact `vip-e.net` zone/account, verifies the same remotely managed production tunnel, writes the exact root-only activation credential, then runs the fixed operator in read-only `--check` mode. Syntax/non-interactive safety tests PASS; a non-interactive run exits fail-closed and leaves no secret file.
- Next external action: create one Cloudflare custom API token scoped only to the AIONEX account and `vip-e.net` with Account `Cloudflare Tunnel: Edit`, Zone `DNS: Edit`, and Zone `Zone: Read`; install it through the root-only helper, run activation check/apply, then repeat external six-locale portal acceptance. Only after `ai.vip-e.net` serves the current portal may this User Campaign Advisor production closeout be marked complete. Live advertising legal acknowledgement and launch authorization remain separate later GS-12 gates.
