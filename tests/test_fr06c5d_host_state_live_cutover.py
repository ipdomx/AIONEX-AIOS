...
def test_executor_has_prestart_only_rollback_path():
 t=(ROOT/'scripts/security/fr06c5_host_state_cutover.py').read_text();assert 'if not started' in t;assert 'blind rollback is prohibited' in t
...
def test_divergence_boundary_precedes_explicit_start_batch():
    text=(ROOT/'scripts/security/fr06c5_host_state_cutover.py').read_text()
    needle="run(['systemctl','start','docker.socket','docker.service'],180);started=True;run(['docker','start'"
...
def test_nested_mounts_are_rejected_before_precopy():
    ...
    assert 'reject_nested_mounts();precopy()' in text
