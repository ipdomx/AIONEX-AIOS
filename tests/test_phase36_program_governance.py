...
def test_phase36_batches_are_complete_registry_and_36a_closes_first() -> None:
    ...
    assert BATCHES[4].status == "in_progress"
    assert all(batch.status == "planned" for batch in BATCHES[5:])
...
def test_phase36_snapshot_is_truthful_and_phase29_is_not_current_finality() -> None:
    ...
    assert snapshot["current_batch"] == "36E"
    ...
    maturity = {item.capability_id: item.maturity for item in CAPABILITIES}
    assert maturity["creative-asset-graph"] == "runtime_verified"
    assert maturity["media-render-transcode"] == "runtime_verified"
    assert maturity["object-storage-media"] == "runtime_verified"
