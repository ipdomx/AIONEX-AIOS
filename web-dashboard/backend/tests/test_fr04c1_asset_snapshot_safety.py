...
def test_platform_asset_snapshot_rejects_hardlinked_files(tmp_path: Path) -> None:
    backup_dir = _private_dir(tmp_path / "backups")
    source_root = _private_dir(tmp_path / "media-assets")
    database = _private_file(
        backup_dir / f"backup-{'a' * 24}-{'b' * 32}.dump",
        b"PGDMPdatabase",
    )
    original = _private_file(source_root / "tenant-a" / "asset.bin", b"asset")
    hardlink = source_root / "tenant-a" / "asset-copy.bin"
    ...
    config = Settings(
        ...
        MEDIA_STORAGE_ROOT=str(source_root),
    )

    with pytest.raises(BackupExecutionError, match="unsafe hard link"):
