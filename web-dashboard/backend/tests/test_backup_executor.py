...
def test_production_images_ship_worker_and_credential_gate_once() -> None:
    ...
    for compose, expected_backend_images in ((primary_compose, 16), (deploy_compose, 15)):
        ...
        assert "audio-speech-worker:" in compose
        audio_section = compose.split("audio-speech-worker:", 1)[1].split(
        ...
        assert "audio-transcript-worker:" in compose
        transcript_section = compose.split("audio-transcript-worker:", 1)[1].split(
            "video-provider-worker:", 1
        )[0]
        assert 'profiles: ["audio-execution"]' in transcript_section
        assert (
        ...
        assert "audio-dubbing-worker:" in compose
        dubbing_section = compose.split("audio-dubbing-worker:", 1)[1].split(
            "video-provider-worker:", 1
        )[0]
        ...
        assert "audio-music-worker:" in compose
        music_section = compose.split("audio-music-worker:", 1)[1].split(
            "video-provider-worker:", 1
        )[0]
