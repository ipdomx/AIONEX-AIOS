...
async def test_owner_can_read_identity_catalogues() -> None:
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        assert (await client.get("/api/v1/organizations", headers=headers)).status_code == 200
        assert (await client.get("/api/v1/roles", headers=headers)).status_code == 200
        assert (await client.get("/api/v1/permissions", headers=headers)).status_code == 200
...
async def test_owner_can_create_org_role_and_user() -> None:
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        if organization_response.status_code == 201:
            ...
            organizations = (await client.get("/api/v1/organizations", headers=headers)).json()
