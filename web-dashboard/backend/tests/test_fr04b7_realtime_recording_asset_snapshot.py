...
def test_realtime_recordings_join_platform_asset_snapshot(tmp_path: Path) -> None:
    ...
    _private_file(
        Path(config.REALTIME_RECORDING_ROOT) / "room-a" / "recording.mp4",
        b"realtime-video",
        realtime=True,
    )
    ...
    assert snapshot.roots["realtime_recording_data"] == {
        "file_count": 1,
        "payload_bytes": len(b"realtime-video"),
    }
    ...
    assert "realtime_recording_data/room-a/recording.mp4" in names


def test_realtime_recording_snapshot_rejects_symlinks(tmp_path: Path) -> None:
    config = _settings(tmp_path)
    backup_dir = Path(config.BACKUP_DIR)
    database = _private_file(
    ...
    target = _private_file(tmp_path / "outside.mp4", b"outside")
    link = Path(config.REALTIME_RECORDING_ROOT) / "room-a" / "leak.mp4"
    link.parent.mkdir(parents=True, exist_ok=True)
    os.chmod(link.parent, 0o2770)
...
def test_realtime_recording_snapshot_rejects_world_readable_files(tmp_path: Path) -> None:
    config = _settings(tmp_path)
    backup_dir = Path(config.BACKUP_DIR)
    database = _private_file(
    ...
    public = _private_file(
        Path(config.REALTIME_RECORDING_ROOT) / "room-a" / "public.mp4",
        b"public",
        realtime=True,
    )
    os.chmod(public, 0o664)

    with pytest.raises(BackupExecutionError, match="unsafe file permissions"):
        ThreeDAssetSnapshotExecutor(config).create_snapshot(str(database))


def test_backup_worker_mounts_realtime_recordings_read_only() -> None:
