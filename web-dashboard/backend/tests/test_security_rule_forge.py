...
from app.services.security_rule_forge import (
    _detector_for,
    evaluate_detector,
    validation_corpus,
)
...
def test_source_marker_detector_does_not_embed_secret_value():
    _, detector = _detector_for(
        finding(
            source="aionex-secrets",
            title="Potential generic secret",
            category="secret-exposure",
            evidence={"marker": "generic-secret-assignment", "line": 12},
        )
    )
    assert detector == {
        "kind": "source_marker",
        "marker": "generic-secret-assignment",
        "category": "secret-exposure",
    }
