...
def test_portal_assets_join_platform_asset_snapshot(tmp_path: Path) -> None:
    ...
    _private_file(
        Path(config.PORTAL_ASSET_ROOT) / "tenant-a" / "hero.webp",
        b"portal-hero",
    )
    ...
    assert snapshot.roots["portal_asset_data"] == {
        "file_count": 1,
        "payload_bytes": len(b"portal-hero"),
    }
    ...
    assert "portal_asset_data/tenant-a/hero.webp" in names


def test_portal_asset_snapshot_rejects_symlinks(tmp_path: Path) -> None:
    config = _settings(tmp_path)
    backup_dir = Path(config.BACKUP_DIR)
    database = _private_file(
    ...
    target = _private_file(tmp_path / "outside.webp", b"outside")
    link = Path(config.PORTAL_ASSET_ROOT) / "tenant-a" / "leak.webp"
...
def test_portal_entrypoint_accepts_prepared_read_only_root() -> None:
