...
async def test_music_pipeline_is_draft_first_unarmed_and_exact_cost_gated(
    ...
    try:
        ...
        async with SessionLocal() as session:
            ...
            assert row.model == "google/lyria-3"
...
async def test_completed_music_is_stored_hash_only_and_opens_local_dag(
    ...
    try:
        ...
        assert claim is not None
        await authority.mark_submission_started(claim)
        await authority.mark_submitted(
            claim,
            provider_request_id="prediction-001",
            provider_response_metadata={"prediction_status": "succeeded"},
        )
        claim = await authority.claim()
        ...
        async with SessionLocal() as session:
            ...
            assert "prediction-001" not in rendered
    finally:
        await cleanup(scope)
...
async def test_replicate_prediction_id_is_durable_and_reclaimed_for_poll_without_resubmit(
    ...
    try:
        ...
        await worker_a.mark_submitted(
            first,
            provider_request_id="prediction-001",
            provider_response_metadata={"prediction_status": "starting"},
        )
        ...
        count = await worker_b.mark_poll_pending(
            second,
            provider_response_metadata={"prediction_status": "processing"},
            delay_seconds=1,
            max_polls=20,
        )
        assert count == 1
