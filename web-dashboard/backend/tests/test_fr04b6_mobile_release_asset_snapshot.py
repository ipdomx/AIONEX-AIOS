...
def test_mobile_releases_join_platform_asset_snapshot(tmp_path: Path) -> None:
    ...
    _private_file(
        Path(config.MOBILE_RELEASE_ROOT) / "ios" / "manifest.json",
        b'{"build":"mobile"}',
    )
    ...
    assert snapshot.roots["mobile_release_data"] == {
        "file_count": 1,
        "payload_bytes": len(b'{"build":"mobile"}'),
    }
    ...
    assert "mobile_release_data/ios/manifest.json" in names


def test_mobile_release_snapshot_rejects_symlinks(tmp_path: Path) -> None:
    config = _settings(tmp_path)
    backup_dir = Path(config.BACKUP_DIR)
    database = _private_file(
    ...
    target = _private_file(tmp_path / "outside.apk", b"outside")
    link = Path(config.MOBILE_RELEASE_ROOT) / "android" / "leak.apk"
...
def test_mobile_entrypoint_accepts_prepared_read_only_root() -> None:
