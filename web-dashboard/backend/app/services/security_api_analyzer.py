...
_ID_NAMES = {"id", "user_id", "project_id", "workspace_id", "organization_id", "order_id", "payment_id", "resource_id"}
_PUBLIC_HINTS = {"/auth/login", "/auth/register", "/auth/forgot", "/health", "/ready", "/portal"}
...
def analyze_openapi(spec: dict[str, Any]) -> dict[str, Any]:
    ...
    for path, raw_path in paths.items():
        if not isinstance(raw_path, dict):
            continue
        path_parameters = raw_path.get("parameters") if isinstance(raw_path.get("parameters"), list) else []
        for method, operation in raw_path.items():
            method_lower = str(method).lower()
            if method_lower not in {"get", "post", "put", "patch", "delete", "options", "head"} or not isinstance(operation, dict):
                continue
            operations += 1
            security = operation.get("security", global_security)
            likely_public = any(str(path).startswith(prefix) for prefix in _PUBLIC_HINTS)
            if method_lower in _MUTATING and not likely_public and (security is None or security == []):
                findings.append({
                    ...
                    "confidence": 0.72,
                    "state": "observed",
                    "fingerprint": _fingerprint(str(path), method_lower, "no-security"),
                    "cwe": "CWE-306",
                    "owasp": "API2:2023",
            ...
            for candidate in [*path_parameters, *(operation.get("parameters") or [])]:
                if isinstance(candidate, dict):
                    parameters.append(candidate)
            object_param = any(
                str(item.get("in") or "") == "path"
                and (str(item.get("name") or "").lower() in _ID_NAMES or str(item.get("name") or "").lower().endswith("_id"))
            ...
            if object_param and not likely_public and "403" not in responses:
                findings.append({
                    ...
                    "confidence": 0.45,
                    "state": "observed",
                    "fingerprint": _fingerprint(str(path), method_lower, "object-authz-contract"),
                    "cwe": "CWE-639",
                    "owasp": "API1:2023",
                    "location": f"{method_lower.upper()} {path}",
                    "evidence": {"method": method_lower.upper(), "path": path, "documented_responses": sorted(map(str, responses))},
                    "remediation": "Verify object-level authorization in implementation and document 403/404 behavior in the API contract.",
                })
    schemes = ((spec.get("components") or {}).get("securitySchemes") or {}) if isinstance(spec, dict) else {}
    if not schemes:
        findings.append({
            "source": "aionex-api-contract",
            "category": "api-authentication",
    ...
    return {"scanner": "aionex-api-contract-v1", "operations": operations, "findings": findings}
