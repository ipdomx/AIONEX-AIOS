...
def normalize_three_d_policy(value: dict[str, Any] | None) -> dict[str, Any]:
    ...
    plans = sorted({str(x).strip().lower() for x in raw.get("allowed_plan_codes", []) if str(x).strip()})
    allowed = sorted({str(x).strip() for x in raw.get("allowed_user_ids", []) if str(x).strip()})
    denied = sorted({str(x).strip() for x in raw.get("denied_user_ids", []) if str(x).strip()})
    policy = {
        "enabled": bool(raw["enabled"]),
        "allowed_plan_codes": plans or ["business"],
        "required_entitlement": str(raw.get("required_entitlement") or THREE_D_ENTITLEMENT).strip(),
        "allowed_user_ids": allowed,
        "denied_user_ids": denied,
        "max_concurrent_jobs_per_user": max(1, min(int(raw["max_concurrent_jobs_per_user"]), 4)),
        ...
        "max_estimated_job_cost_usd": max(0.01, min(float(raw["max_estimated_job_cost_usd"]), 100.0)),
        "daily_spend_limit_usd": max(0.01, min(float(raw["daily_spend_limit_usd"]), 10000.0)),
        "monthly_spend_limit_usd": max(0.01, min(float(raw["monthly_spend_limit_usd"]), 100000.0)),
        "owner_alert_threshold_pct": max(1, min(int(raw["owner_alert_threshold_pct"]), 100)),
    }
    if policy["required_entitlement"] == "":
        policy["required_entitlement"] = THREE_D_ENTITLEMENT
    return policy

def three_d_access_allowed(policy: dict[str, Any], *, user_id: str, plan_code: str, entitlements: list[str]) -> bool:
...
async def _record(session: AsyncSession, *, lock: bool = False) -> OwnerControlRecord:
    ...
    if record is None:
        await session.execute(pg_insert(OwnerControlRecord).values(
            id=uuid_str(), domain=THREE_D_POLICY_DOMAIN, resource_id=THREE_D_POLICY_RESOURCE,
            status="active", enabled=True, payload=DEFAULT_THREE_D_POLICY, version=1,
        ).on_conflict_do_nothing(constraint="uq_owner_control_domain_resource"))
        record = await session.scalar(stmt)
    if record is None:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="3D service policy unavailable")
    normalized = normalize_three_d_policy(record.payload)
    if record.payload != normalized:
        record.payload = normalized; record.version += 1
    return record
...
async def update_three_d_policy(session: AsyncSession, updates: dict[str, Any]) -> dict[str, Any]:
