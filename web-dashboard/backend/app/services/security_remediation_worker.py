...
def _copy_source(
    ...
    *,
    ...
    allowed = []
    for configured in (SOURCE_ROOT, PROJECT_EXECUTION_ROOT):
        try:
            allowed.append(configured.resolve(strict=True))
        except FileNotFoundError:
            continue
    if not any(source == root or root in source.parents for root in allowed):
        raise ValueError("Remediation source is outside the Security Lab source roots")
...
class Worker:
    def __init__(self) -> None:
        ...
        self.poll = max(
            1, int(os.getenv("SECURITY_REMEDIATION_WORKER_POLL_SECONDS", "5"))
        )
    ...
    def write_health(self, status: str) -> None:
        ...
        tmp = self.health.with_name("." + self.health.name + ".tmp")
        tmp.write_text(json.dumps(payload, sort_keys=True), encoding="utf-8")
        os.chmod(tmp, 0o600)
        os.replace(tmp, self.health)
    ...
    async def claim(self) -> tuple[str, str] | None:
        ...
        async with SessionLocal() as session:
            item = await session.scalar(
                ...
                .where(
                    or_(
                        SecurityRemediation.status == "planned",
                        and_(
                            SecurityRemediation.status == "preparing",
                            SecurityRemediation.updated_at < stale,
                        ),
                    )
                )
                .order_by(SecurityRemediation.created_at)
            ...
            item.worktree_ref = f"preparing:{token}"
        ...
        async with SessionLocal() as session:
            item = await session.scalar(
                ...
                .where(
                    ...
                    SecurityRemediation.status == "preparing",
                    SecurityRemediation.worktree_ref == f"preparing:{token}",
                )
                .with_for_update()
            )
            if item is None:
            ...
            target = (
                await session.get(SecurityTarget, finding.target_id)
                if finding
                else None
            )
            if (
            ...
            raw = str(
                (target.target_metadata or {}).get("source_snapshot") or ""
            ).strip()
            if not raw:
                raise RuntimeError("Managed target has no isolated source snapshot")
            source = Path(raw)
            destination = WORK_ROOT / item.id / "source"
            ...
            session.add(
                AuditEvent(
                    ...
                    resource_id=item.id,
                    details={**evidence, "production_modified": False},
                )
            )
            await session.commit()

    async def fail(self, remediation_id: str, token: str, error_type: str) -> None:
        async with SessionLocal() as session:
            item = await session.scalar(
                select(SecurityRemediation)
                .where(
            ...
            session.add(
                AuditEvent(
                    ...
                    resource_type="security_remediation",
                    resource_id=item.id,
                    details={"error_type": error_type, "production_modified": False},
                )
            )
            await session.commit()
    ...
    async def run(self) -> None:
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                if claimed:
                    await self.prepare(*claimed)
                self.write_health("healthy")
            except Exception as exc:
                if claimed:
                    await self.fail(claimed[0], claimed[1], type(exc).__name__)
                self.errors += 1
                logger.exception("Security remediation worker cycle failed")
...
def healthcheck() -> int:
    ...
    try:
        ...
        return (
            ...
            and time.time() - float(data["checked_at_epoch"]) < 90
            else 1
        )
    except Exception:
        return 1
