...
def _detector_for(finding: SecurityFinding) -> tuple[str, dict[str, Any]]:
    ...
    if finding.category == "csp" and evidence.get("directive"):
        return "http-header", {
            "kind": "csp_forbidden_directive",
            "directive": str(evidence["directive"]).lower(),
        }
    if finding.source in {"aionex-source", "aionex-secrets"} and evidence.get("marker"):
        return "source-pattern", {
            "kind": "source_marker",
            "marker": str(evidence["marker"]),
    ...
    return "aionex-policy", {
        "kind": "finding_fingerprint",
...
async def derive_candidate(
    session: AsyncSession, actor: UserRecord, finding: SecurityFinding
) -> SecurityRule:
    if actor.role != "Super Owner":
        raise PermissionError(
            "Only the Super Owner can promote Security Genome candidates"
        )
    ...
    signature = hashlib.sha256(
        canonical({"type": rule_type, "detector": detector}).encode()
    ).hexdigest()
    existing = await session.scalar(
        select(SecurityRule).where(
            SecurityRule.organization_id == actor.organization_id,
            SecurityRule.signature == signature,
        )
    )
    ...
    rule = SecurityRule(
        ...
        rule_type=rule_type,
        name=f"AIONEX: {finding.title}"[:240],
        signature=signature,
        detector=detector,
        status="quarantine",
        trust_score=_trust_from_finding(finding),
        validation_passes=0,
    ...
    session.add(
        AuditEvent(
            organization_id=actor.organization_id,
            user_id=actor.id,
            action="security.rule.candidate_created",
...
def evaluate_detector(detector: dict[str, Any], sample: dict[str, Any]) -> bool:
    ...
    if kind == "missing_header":
        headers = {
            str(k).lower(): str(v) for k, v in dict(sample.get("headers") or {}).items()
        }
    ...
    if kind == "nuclei_template":
        return str(sample.get("template_id") or "") == str(
            detector.get("template_id") or ""
        )
    if kind == "api_contract":
        return str(sample.get("category") or "") == str(
            detector.get("category") or ""
        ) and str(sample.get("title") or "") == str(detector.get("title") or "")
    if kind == "finding_fingerprint":
        return str(sample.get("source") or "") == str(
            detector.get("source") or ""
        ) and str(sample.get("category") or "") == str(detector.get("category") or "")
    return False
...
def validation_corpus(
    ...
    kind = detector.get("kind")
    if kind == "missing_header":
        header = str(detector["header"])
        return ({"headers": {}}, {"headers": {header: "present"}})
    if kind == "csp_forbidden_directive":
        directive = str(detector["directive"])
        return (
            {"csp": f"script-src 'self' '{directive}'"},
            {"csp": "script-src 'self'"},
        )
    if kind == "source_marker":
        marker = str(detector["marker"])
        return ({"markers": [marker]}, {"markers": ["safe-marker"]})
    if kind == "nuclei_template":
        template = str(detector["template_id"])
        return ({"template_id": template}, {"template_id": "different-template"})
    if kind == "api_contract":
        positive = {
            "category": detector.get("category"),
            "title": detector.get("title"),
        }
        return (positive, {"category": "safe", "title": "safe"})
    return (
        {"source": detector.get("source"), "category": detector.get("category")},
        {"source": "different", "category": "different"},
    )
...
async def validate_candidate(
    ...
    rule: SecurityRule,
    *,
    corpus_id: str = "aionex-generated-v1",
) -> SecurityRule:
    ...
    passed = evaluate_detector(rule.detector, positive) and not evaluate_detector(
        rule.detector, negative
    )
    session.add(
        SecurityRuleValidation(
            id=uuid_str(),
            ...
            corpus_id=corpus_id,
            evidence={
                "positive_detected": evaluate_detector(rule.detector, positive),
                "negative_detected": evaluate_detector(rule.detector, negative),
            },
        )
    )
    if passed:
        ...
        if (
            rule.validation_passes >= 1
            and rule.validation_failures == 0
            and rule.trust_score >= 0.80
        ):
    ...
    session.add(
        AuditEvent(
            ...
            user_id=actor.id,
            action=(
                "security.rule.validated"
                if passed
                else "security.rule.validation_failed"
            ),
            resource_type="security_rule",
            resource_id=rule.id,
            details={"corpus_id": corpus_id, "trust_score": rule.trust_score},
        )
    )
    return rule
...
async def promote_rule(
    session: AsyncSession, actor: UserRecord, rule: SecurityRule
) -> SecurityRule:
    if actor.role != "Super Owner" or rule.organization_id != actor.organization_id:
        raise PermissionError("Only the Super Owner can promote Security Genome rules")
    if (
        rule.status != "validated"
        or rule.trust_score < 0.80
        or rule.validation_failures
    ):
    ...
    item = await knowledge_learning.ingest_item(
        ...
        confidence=rule.trust_score,
        tags=["security", "rule", "genome"],
        provenance=[
            {
                "source": f"security-rule:{rule.id}",
                "source_type": "security_scan",
                "source_quality": rule.trust_score,
    ...
    await knowledge_learning.verify_item(
        ...
        accepted=True,
        confidence=rule.trust_score,
        note="Promoted from validated Security Genome rule",
    )
    session.add(
        AuditEvent(
            organization_id=actor.organization_id,
            user_id=actor.id,
            action="security.rule.promoted",
            resource_type="security_rule",
            resource_id=rule.id,
            details={"knowledge_item_id": item.id, "trust_score": rule.trust_score},
        )
    )
    return rule
