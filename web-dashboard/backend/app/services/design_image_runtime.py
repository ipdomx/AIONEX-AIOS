...
def _inspect_raster(body: bytes, output_format: str) -> tuple[int, int]:
    """Validate the canonical raster envelope without external image libraries."""
    if output_format == "png":
        if len(body) < 24 or body[:8] != b"\x89PNG\r\n\x1a\n" or body[12:16] != b"IHDR":
            raise DesignImageExecutionError("provider output is not a valid PNG envelope")
        width, height = struct.unpack(">II", body[16:24])
    elif output_format == "jpeg":
        if len(body) < 4 or body[:2] != b"\xff\xd8":
            raise DesignImageExecutionError("provider output is not a valid JPEG envelope")
        pos = 2
        width = height = 0
        ...
        if not width or not height:
            raise DesignImageExecutionError("provider JPEG dimensions are unavailable")
    elif output_format == "webp":
        if len(body) < 30 or body[:4] != b"RIFF" or body[8:12] != b"WEBP":
            raise DesignImageExecutionError("provider output is not a valid WebP envelope")
        chunk = body[12:16]
        if chunk == b"VP8X":
            ...
            raise DesignImageExecutionError("provider WebP dimensions are unavailable")
    else:
        raise DesignImageExecutionError("design image output format is unsupported")
    if not (1 <= width <= 16384 and 1 <= height <= 16384):
        raise DesignImageExecutionError("provider image dimensions are outside the allowed range")
    return width, height
