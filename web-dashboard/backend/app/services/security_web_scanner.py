...
SECURITY_HEADERS = {
    ...
    "content-security-policy": (
        "high",
        "Deploy a restrictive Content-Security-Policy and remove unsafe directives where practical.",
    ),
    ...
    "permissions-policy": (
        "low",
        "Set Permissions-Policy to disable browser capabilities not required by the application.",
    ),
}
...
def _fingerprint(source: str, category: str, title: str, location: str = "") -> str:
    return hashlib.sha256(
        f"{source}|{category}|{title}|{location}".encode()
    ).hexdigest()
...
def _tls_probe(hostname: str, port: int) -> dict[str, Any]:
    ...
    with socket.create_connection((hostname, port), timeout=8) as raw:
        with context.wrap_socket(raw, server_hostname=hostname) as connection:
            cert: dict[str, Any] = dict(connection.getpeercert() or {})
...
def analyze_tls(
    origin: str, tls: dict[str, Any] | None, error: str | None = None
) -> list[dict[str, Any]]:
    ...
    if error or tls is None:
        findings.append(
            finding(
                ...
                severity="high",
                confidence=0.90,
                evidence={"error_type": error or "unknown"},
                remediation="Fix the certificate chain, hostname coverage, supported protocols, and TLS service availability.",
                cwe="CWE-295",
            )
        )
        return findings
            ...
            if days < 0:
                findings.append(
                    finding(
                        ...
                        remediation="Replace the expired certificate immediately and verify automated renewal.",
                        cwe="CWE-295",
                    )
                )
            elif days < 30:
                findings.append(
                    finding(
                        ...
                        confidence=0.99,
                        evidence={"days_remaining": days},
                        remediation="Renew the certificate and verify the renewal automation before expiry.",
                    )
                )
        except ValueError:
            # An unusual certificate date format is retained as neutral evidence;
            # protocol/cipher validation below still runs.
            not_after = None
    ...
    if version and version not in {"TLSv1.2", "TLSv1.3"}:
        findings.append(
            finding(
                ...
                remediation="Disable legacy TLS versions and require TLS 1.2 or later.",
                cwe="CWE-326",
            )
        )
    return findings
...
def analyze_headers(origin: str, headers: Mapping[str, str]) -> list[dict[str, Any]]:
    ...
    if origin.startswith("https://") and hsts and "max-age=" not in hsts:
        findings.append(
            finding(
                ...
                remediation="Set a valid Strict-Transport-Security max-age directive.",
                cwe="CWE-693",
                location=origin,
            )
        )
        ...
        if "'unsafe-eval'" in lowered:
            findings.append(
                finding(
                    ...
                    remediation="Remove 'unsafe-eval' and migrate dynamic code execution to nonce/hash-based scripts.",
                    cwe="CWE-79",
                    location=origin,
                )
            )
        if "script-src" in lowered and "*" in lowered:
            findings.append(
                finding(
                    source="aionex-headers",
                    category="csp",
                    title="CSP script-src contains wildcard sources",
                    severity="medium",
                    confidence=0.90,
                    evidence={"directive": "script-src"},
                    remediation="Replace wildcard script sources with explicit trusted origins/nonces/hashes.",
                    cwe="CWE-79",
                    location=origin,
                )
            )
        ...
        if "secure" not in cookie:
            findings.append(
                finding(
                    ...
                    remediation="Mark authentication and sensitive cookies Secure.",
                    cwe="CWE-614",
                    location=origin,
                )
            )
        if "httponly" not in cookie:
            findings.append(
                finding(
                    ...
                    confidence=0.90,
                    evidence={"cookie": cookie_name},
                    remediation="Mark session/authentication cookies HttpOnly unless script access is explicitly required.",
                    cwe="CWE-1004",
                    location=origin,
                )
            )
        if "samesite=" not in cookie:
            findings.append(
                finding(
                    ...
                    confidence=0.90,
                    evidence={"cookie": cookie_name},
                    remediation="Set SameSite=Lax or Strict unless a documented cross-site flow requires None+Secure.",
                    cwe="CWE-1275",
                    location=origin,
                )
            )
...
async def scan_web_origin(origin: str) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient(
        timeout=12.0, follow_redirects=True, max_redirects=5
    ) as client:
        try:
            response = await client.get(
                origin + "/",
                headers={
                    "User-Agent": "AIONEX-Security-Lab/1.0",
            ...
            return {
                ...
                + [
                    finding(
                        ...
                        title="HTTP endpoint could not be reached",
                        severity="medium",
                        confidence=0.90,
                        evidence={"error_type": type(exc).__name__},
                        remediation="Confirm DNS, routing, TLS, reverse proxy and application health.",
                        location=origin,
                    )
                ],
            }
    ...
    if final_url.startswith("http://"):
        findings.append(
            finding(
                source="aionex-headers",
                category="transport-security",
                title="Redirect chain ended on HTTP",
                severity="high",
                confidence=0.99,
                evidence={"final_scheme": "http"},
                remediation="Ensure all canonical redirects terminate on HTTPS.",
                cwe="CWE-319",
                location=origin,
            )
        )
