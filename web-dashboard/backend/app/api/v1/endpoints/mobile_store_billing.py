...
class AppStoreNotificationRequest(BaseModel):
    signedPayload: str = Field(min_length=20, max_length=262144)
...
async def app_store_notification(
    data: AppStoreNotificationRequest, session: AsyncSession = Depends(get_db)
):
    return await mobile_store_billing.process_app_store_notification(
        session, data.signedPayload
    )
...
async def readiness(actor: UserRecord = Depends(require_permissions("billing:read"))):
...
async def restore(
    store: StoreName, actor: UserRecord = Depends(require_permissions("billing:write"))
):
    return {
        "store": store,
...
async def reconcile(
    ...
    return await mobile_store_billing.reconcile_user_store(
        session, actor=actor, store=store
    )
...
async def owner_store_mapping_status(
    mapping_id: str,
    data: OwnerStoreMappingStatusRequest,
    actor: UserRecord = Depends(require_super_owner),
    session: AsyncSession = Depends(get_db),
):
    del actor
