...
def upgrade() -> None:
    # The historical initial migration calls current Base.metadata.create_all().
    # A brand-new database can therefore already contain these tables before
    # Alembic reaches this revision. Existing production databases at 0015 do
    # not. Treat the all-present case as already materialized, and fail closed
    # on a partial set instead of silently creating an inconsistent schema.
    ...
    if present:
        raise RuntimeError(
            "Security Learning Fabric schema is partially present; manual review is required "
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "organization_id", "user_id", name="uq_security_grant_org_user"
        ),
    )
    op.create_index(
        "ix_security_grants_status_expiry",
        "security_access_grants",
        ["status", "expires_at"],
    )
    op.create_index(
        "ix_security_access_grants_organization_id",
        "security_access_grants",
        ["organization_id"],
    )
    op.create_index(
        "ix_security_access_grants_user_id", "security_access_grants", ["user_id"]
    )
    ...
    op.create_table(
        ...
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False),
        sa.UniqueConstraint(
            "organization_id", "origin", name="uq_security_target_org_origin"
        ),
    )
    op.create_index(
        "ix_security_targets_org_auth",
        "security_targets",
        ["organization_id", "authorization_status", "status"],
    )
    for name in (
        "organization_id",
        "project_id",
    ...
    op.create_table(
        ...
        sa.Column(
            "target_id",
            sa.String(36),
            sa.ForeignKey("security_targets.id", ondelete="CASCADE"),
            nullable=False,
        ),
        sa.Column(
            "requested_by_id",
    ...
    op.create_index(
        "ix_security_scans_org_status_created",
        "security_scans",
        ["organization_id", "status", "created_at"],
    )
    op.create_index(
        "ix_security_scans_target_created",
        "security_scans",
    ...
    op.create_table(
        ...
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False),
        sa.UniqueConstraint(
            "scan_id", "fingerprint", name="uq_security_finding_scan_fingerprint"
        ),
    )
    op.create_index(
        "ix_security_findings_org_state_severity",
        "security_findings",
        ["organization_id", "state", "severity"],
    )
    op.create_index(
        "ix_security_findings_target_created",
        "security_findings",
        ["target_id", "created_at"],
    )
    for name in (
    ...
    op.create_table(
        ...
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False),
        sa.UniqueConstraint(
            "organization_id", "signature", name="uq_security_rule_org_signature"
        ),
    )
    op.create_index(
        "ix_security_rules_status_trust", "security_rules", ["status", "trust_score"]
    )
    for name in (
        "organization_id",
        "source_finding_id",
    ...
    op.create_table(
        ...
        sa.Column(
            "rule_id",
            sa.String(36),
            sa.ForeignKey("security_rules.id", ondelete="CASCADE"),
            nullable=False,
        ),
    ...
    op.create_index(
        "ix_security_rule_validations_rule_created",
        "security_rule_validations",
        ["rule_id", "created_at"],
    )
    op.create_index(
        "ix_security_rule_validations_rule_id", "security_rule_validations", ["rule_id"]
    )
    ...
    op.create_index(
        "ix_security_remediations_org_status_created",
        "security_remediations",
        ["organization_id", "status", "created_at"],
    )
    for name in (
        ...
        op.create_index(
            f"ix_security_remediations_{name}", "security_remediations", [name]
        )
    ...
    op.create_index(
        "ix_security_release_gate_project_created",
        "security_release_gates",
    ...
    for name in (
        ...
        op.create_index(
            f"ix_security_release_gates_{name}", "security_release_gates", [name]
        )
