"""Add Phase 36F durable asynchronous video execution authority.
...
def upgrade() -> None:
    ...
    required_tables = {
        ...
        "workspaces", "projects", "studio_jobs", "studio_assets",
    }
    missing = sorted(required_tables - tables)
    if missing:
        raise RuntimeError("Phase 36F video migration requires tables: " + ", ".join(missing))
    ...
    if "video_executions" not in tables:
        op.create_table(
            ...
            sa.Column("provider_job_id", sa.String(240)),
            ...
            sa.UniqueConstraint("organization_id", "idempotency_key", name="uq_video_execution_org_idempotency"),
            sa.UniqueConstraint("provider", "provider_job_id", name="uq_video_execution_provider_job"),
        )
    else:
        required_columns = {
            "id", "organization_id", "graph_id", "target_node_id", "requested_by_id",
            "scene_key", "operation", "provider", "model", "status", "idempotency_key",
            "prompt_sha256", "request_options", "output_format", "attempts", "max_attempts",
            "poll_count", "max_polls", "lease_token", "lease_owner", "lease_expires_at",
            "fencing_token", "available_at", "provider_job_id", "provider_state",
            "provider_progress", "provider_response_metadata", "usage_metadata",
            "estimated_cost_usd", "actual_cost_usd", "cost_basis", "output_storage_backend",
            "output_storage_key", "output_checksum", "output_size_bytes", "armed_at",
            "provider_submitted_at", "last_polled_at", "started_at", "completed_at",
            "created_at", "updated_at",
        }
        existing = {item["name"] for item in inspector.get_columns("video_executions")}
        missing = sorted(required_columns - existing)
        if missing:
            raise RuntimeError("Existing video_executions table is incomplete: " + ", ".join(missing))

    existing_indexes = {item["name"] for item in sa.inspect(bind).get_indexes("video_executions")}
    for name, fields in (
        ...
        ("ix_video_executions_target_node_id", ["target_node_id"]),
        ("ix_video_executions_requested_by_id", ["requested_by_id"]),
        ("ix_video_executions_scene_key", ["scene_key"]),
        ("ix_video_executions_operation", ["operation"]),
        ("ix_video_executions_provider", ["provider"]),
        ("ix_video_executions_status", ["status"]),
        ("ix_video_executions_prompt_sha256", ["prompt_sha256"]),
        ("ix_video_executions_lease_owner", ["lease_owner"]),
        ("ix_video_executions_lease_expires_at", ["lease_expires_at"]),
        ("ix_video_executions_available_at", ["available_at"]),
        ("ix_video_executions_provider_job_id", ["provider_job_id"]),
        ("ix_video_executions_provider_state", ["provider_state"]),
        ("ix_video_executions_output_checksum", ["output_checksum"]),
        ("ix_video_executions_org_status_created", ["organization_id", "status", "created_at"]),
        ("ix_video_executions_claim", ["status", "available_at", "created_at"]),
        ("ix_video_executions_graph_status", ["graph_id", "status"]),
        ("ix_video_executions_provider_state_lookup", ["provider", "provider_state"]),
    ):
