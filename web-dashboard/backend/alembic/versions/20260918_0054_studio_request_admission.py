"""Add request-only Production Studio admission without claiming execution coverage.

Revision ID: 20260918_0054
Revises: 20260918_0053

Only a validated schema-6 authority advances. Preserve closure, operation, reason
...
def _valid_schema_six(row) -> bool:
    # Frozen validation: do not import mutable future application validators.
    payload = row["payload"]
    if not isinstance(payload, dict) or set(payload) != _PAYLOAD_KEYS:
        return False
    if (
        type(payload["schema_version"]) is not int or payload["schema_version"] != 6
        or payload["scope"] != "project_execution+backup_cycles+academy_course_packages+notification_delivery_dispatch+security_remediation_preparation+security_scan_requests"
        or payload["full_host_closure"] is not False
        or type(payload["generation"]) is not int or payload["generation"] < 6
...
def upgrade() -> None:
    ...
    if bind.dialect.name != "postgresql":
        raise RuntimeError("Studio request admission requires PostgreSQL")
    ...
    if row is None or not _valid_schema_six(row):
        return
    ...
    if changed != row["id"]:
        raise RuntimeError("Studio request admission migration lost authority generation")
