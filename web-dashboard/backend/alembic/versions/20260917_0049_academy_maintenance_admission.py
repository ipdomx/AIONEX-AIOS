...
def _expand_registry(bind) -> None:
    ...
    if consumer_check in notification_consumers | remediation_consumers:
        ...
        notification_check = checks.get("ck_host_cycle_notification_job", "")
        if _normalized_check(notification_check) != (
            "consumer<>'notification_delivery_dispatch'orjob_idisnotnull"
        ):
            raise RuntimeError("Retained notification activity reference is incompatible")
        indexes = {item["name"]: item for item in sa.inspect(bind).get_indexes(_TABLE)}
        existing = indexes.get("uq_host_cycle_notification_delivery")
        if existing is None:
            raise RuntimeError("Retained notification activity uniqueness is missing")
        predicate = existing.get("dialect_options", {}).get("postgresql_where", "")
        if (
            existing.get("unique") is not True
            or existing.get("column_names") != ["consumer", "job_id"]
            ...
            raise RuntimeError("Retained notification activity uniqueness is incompatible")
    elif consumer_check in accepted_consumers:
