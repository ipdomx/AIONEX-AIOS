"""Retain Studio post-crash containment evidence without clearing blockers.

Revision ID: 20260920_0061
Revises: 20260919_0060

This migration adds an immutable evidence ledger only. It performs no backfill,
filesystem operation, retry, settlement, blocker clearing or production
activation. Existing metadata is accepted only when it matches frozen DDL.
"""
...
revision = "20260920_0061"
down_revision = "20260919_0060"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    if bind.dialect.name != "postgresql":
        raise RuntimeError("Studio crash containment requires PostgreSQL")
    bind.execute(sa.text("SELECT set_config('lock_timeout', '5s', true)"))
    name = "studio_crash_containments"
    if not sa.inspect(bind).has_table(name):
        _table(name).create(bind)
        return
    reference = _table(
        "_fr06_0061_studio_crash_containment_expected",
        temporary=True,
    )
    ...
    try:
        ...
        if _signature(bind, name) != _signature(
            ...
            raise RuntimeError(
                "Existing Studio crash containment ledger does not match "
                "the frozen schema"
            )
    finally:
        reference.drop(bind)
...
def downgrade() -> None:
    raise RuntimeError(
        "Studio crash containment provenance cannot be discarded by downgrade"
    )
