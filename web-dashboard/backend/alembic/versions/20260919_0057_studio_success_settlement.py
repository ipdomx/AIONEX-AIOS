"""Retain acknowledged normal-success Studio settlement without erasing evidence.
...
def _table(name: str, *, temporary: bool = False) -> sa.Table:
    return sa.Table(
        ...
        sa.CheckConstraint("admitted_generation >= 7", name="ck_studio_settlement_generation"),
...
def upgrade() -> None:
    ...
    if bind.dialect.name != "postgresql":
        raise RuntimeError("Studio settlement requires PostgreSQL")
    bind.execute(sa.text("SELECT set_config('lock_timeout', '5s', true)"))
    name = "studio_settlements"
    if not sa.inspect(bind).has_table(name):
        _table(name).create(bind)
        return
    reference = _table("_fr06_0057_studio_settlement_expected", temporary=True)
...
def downgrade() -> None:
    raise RuntimeError("Studio settlement provenance cannot be discarded by downgrade")
