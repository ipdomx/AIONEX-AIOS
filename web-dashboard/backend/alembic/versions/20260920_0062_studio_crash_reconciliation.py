"""Retain terminal Studio crash reconciliation without deleting evidence.

Revision ID: 20260920_0062
Revises: 20260920_0061

This migration adds an immutable terminal-reconciliation evidence ledger only.
It performs no backfill, filesystem operation, retry, cleanup, quarantine/final
deletion, deployment or production activation. Existing metadata is accepted
only when it matches frozen DDL.
"""
...
revision = "20260920_0062"
down_revision = "20260920_0061"
branch_labels = None
depends_on = None
...
def _table(name: str, *, temporary: bool = False) -> sa.Table:
    return sa.Table(
        ...
        sa.Column("containment_id", sa.String(36), nullable=False, unique=True),
        sa.Column("execution_id", sa.String(36), nullable=False, unique=True),
        sa.Column("publication_id", sa.String(36), nullable=False, unique=True),
...
def upgrade() -> None:
    ...
    if bind.dialect.name != "postgresql":
        raise RuntimeError("Studio crash reconciliation requires PostgreSQL")
    bind.execute(sa.text("SELECT set_config('lock_timeout', '5s', true)"))
    name = "studio_crash_reconciliations"
    if not sa.inspect(bind).has_table(name):
        _table(name).create(bind)
        return
    reference = _table(
        "_fr06_0062_studio_crash_reconciliation_expected",
        temporary=True,
    )
    ...
    try:
        ...
        if _signature(bind, name) != _signature(
            bind, reference.name, schema
        ):
            raise RuntimeError(
                "Existing Studio crash reconciliation ledger does not match "
                "the frozen schema"
            )
    finally:
        reference.drop(bind)
...
def downgrade() -> None:
    raise RuntimeError(
        "Studio crash reconciliation evidence cannot be discarded by downgrade"
    )
