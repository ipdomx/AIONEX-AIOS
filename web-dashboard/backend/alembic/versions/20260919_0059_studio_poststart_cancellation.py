"""Retain proven post-start Studio cancellation without erasing evidence.

Revision ID: 20260919_0059
Revises: 20260919_0058

No production activation, backfill, file deletion, replay or crash reconciliation.
An existing ledger is accepted only when its schema exactly matches the frozen DDL.
"""
...
revision = "20260919_0059"
down_revision = "20260919_0058"
branch_labels = None
depends_on = None
...
def _table(name: str, *, temporary: bool = False) -> sa.Table:
    return sa.Table(
        ...
        sa.CheckConstraint(
            ...
            name="ck_studio_poststart_cancel_generation",
        ),
...
def upgrade() -> None:
    ...
    if bind.dialect.name != "postgresql":
        raise RuntimeError("Studio post-start cancellation requires PostgreSQL")
    bind.execute(sa.text("SELECT set_config('lock_timeout', '5s', true)"))
    name = "studio_poststart_cancellations"
    if not sa.inspect(bind).has_table(name):
        _table(name).create(bind)
        return

    reference = _table("_fr06_0059_studio_poststart_expected", temporary=True)
    ...
    try:
        ...
        if _signature(bind, name) != _signature(
            bind, reference.name, temporary_schema
        ):
            raise RuntimeError(
                "Existing post-start cancellation ledger does not match "
                "the frozen schema"
            )
    finally:
        reference.drop(bind)
...
def downgrade() -> None:
    raise RuntimeError(
        "Studio post-start cancellation provenance cannot be discarded by downgrade"
    )
