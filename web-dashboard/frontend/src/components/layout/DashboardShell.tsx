  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-space-950">
      {mobile && mobileSidebarOpen && (
        <button type="button" aria-label="Close navigation" className="fixed inset-0 z-[45] bg-black/70 backdrop-blur-sm" onClick={() => setMobileSidebarOpen(false)} />
      )}
      <Sidebar
        ...
        onNavigate={() => { if (mobile) setMobileSidebarOpen(false); }}
      />
      <TopBar
      ...
      >
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }} className="mx-auto w-full min-w-0 max-w-[1600px]">
          {children}
        </motion.div>
      </motion.main>
      <Link href="/studio" className="fixed bottom-5 left-4 z-40 flex items-center gap-2 rounded-full border border-electric-400/30 bg-space-900/95 px-4 py-2.5 text-sm font-semibold text-white shadow-xl backdrop-blur-xl transition hover:border-electric-300/60 hover:bg-electric-500/15 sm:left-auto sm:right-5">
