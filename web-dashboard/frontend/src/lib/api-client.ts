...
export const AUTH_SESSION_EVENT = "aionex:auth-session";
...
function invalidateBrowserSession(storage: Storage): null {
...
class ApiClient {
  ...
  private async refreshAccessToken(): Promise<string | null> {
    ...
    if (!refreshToken) return invalidateBrowserSession(storage);
      ...
      this.refreshPromise = axios
        ...
        .then((response) => {
          ...
          const nextRefreshToken = response.data?.refresh_token as
            string | undefined;
          const refreshedUser = response.data?.user as
            | Record<string, unknown>
            | undefined;
          if (!accessToken || !refreshedUser)
            return invalidateBrowserSession(storage);
        ...
        .catch(() => invalidateBrowserSession(storage))
        .finally(() => {
