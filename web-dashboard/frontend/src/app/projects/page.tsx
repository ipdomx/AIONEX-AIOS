  ...
  async function loadProjects() {
    ...
    try {
      const [projectItems, workspaceItems] = await Promise.all([
        runtimeServices.listProjects({ limit: 100 }),
        runtimeServices.listWorkspaces(),
      ]);
    ...
    <div className="space-y-6">
      ...
      >
        <div>
          ...
          <p className="text-sm text-white/40 mt-1">
            Live projects loaded from the AIOS backend
          </p>
        </div>
        ...
        <form onSubmit={createProject} className="glass-card p-5 space-y-4">
          ...
          {workspaces.length === 0 && (
            <p className="text-xs text-orange-300">
              Create an active workspace before creating a project.
            </p>
          )}
          <select
            value={priority}
          ...
          >
            ...
            <option value="medium" className="bg-space-800">
              Medium
            </option>
            <option value="high" className="bg-space-800">
              High
            </option>
            <option value="critical" className="bg-space-800">
              Critical
            </option>
          </select>
          <button disabled={creating || !workspaceId} className="btn-primary">
            {creating ? (
      ...
      <div className="flex items-center gap-3">
        ...
        >
          ...
          <option value="active" className="bg-space-800">
            Active
          </option>
          <option value="planning" className="bg-space-800">
            Planning
          </option>
          <option value="paused" className="bg-space-800">
            Paused
          </option>
          <option value="completed" className="bg-space-800">
            Completed
          </option>
        </select>
      </div>
          ...
          {filteredProjects.map((project, index) => (
            ...
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <span
                  className={`px-2 py-0.5 rounded-full text-[10px] font-medium border ${getPriorityColor(project.priority)}`}
                >
                  {project.priority}
                </span>
              </div>
