...
const emptySnapshot: OwnerFinalizationSnapshot = { generatedAt: "", completion: 0, checks: [] };
...
export default function OwnerFinalizationPage() {
  ...
  async function load(signal?: AbortSignal) {
    ...
    try {
      ...
      setSnapshot(data);
      setMessage(`Finalization synchronized at ${new Date(data.generatedAt).toLocaleTimeString()}.`);
    } catch (error) {
      if (!(error instanceof DOMException && error.name === "AbortError")) setMessage("Finalization synchronization failed.");
    } finally {
  ...
  const summary = useMemo(() => ({
    ...
    warnings: snapshot.checks.filter((item) => item.status === "warning").length,
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <div>
          <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-electric-500/20 bg-electric-500/10 px-3 py-1 text-xs font-medium text-electric-300"><ShieldCheck className="h-3.5 w-3.5" /> Owner Dashboard Finalization</div>
          <h1 className="text-3xl font-bold tracking-tight text-white">Production Readiness & Closure</h1>
          <p className="mt-2 text-sm text-white/45">Unified final verification for integration, security, reliability, performance and owner workflows.</p>
        </div>
      ...
      <div className="glass-card p-5">
        <div className="flex items-center justify-between gap-4"><div><div className="text-xs uppercase tracking-wider text-white/30">Overall completion</div><div className="mt-2 text-4xl font-bold text-white">{snapshot.completion}%</div></div><div className="rounded-full border border-green-500/20 bg-green-500/10 px-4 py-2 text-sm text-green-300">Final owner dashboard</div></div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        {snapshot.checks.map((item, index) => (
          <motion.div key={item.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.03 }} className="glass-card p-5">
            <div className="flex items-start justify-between gap-4"><div><h2 className="text-sm font-semibold text-white">{item.label}</h2><p className="mt-1 text-xs text-white/40">{item.category} · {item.details}</p></div><span className={`rounded-full border px-2.5 py-1 text-xs ${statusClass[item.status]}`}>{item.status}</span></div>
          </motion.div>
        ))}
      </div>
    </div>
