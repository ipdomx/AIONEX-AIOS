...
export default function OwnerRuntimePage() {
  ...
  async function load(signal?: AbortSignal) {
    ...
    try {
      ...
      setSnapshot(data);
      setMessage(
        `Runtime synchronized at ${new Date(data.generatedAt).toLocaleTimeString()}.`,
      );
    } catch (error) {
      if (!(error instanceof DOMException && error.name === "AbortError")) {
        setSnapshot(emptySnapshot);
        setMessage("Owner runtime backend contract is not available.");
      }
    } finally {
  ...
  const totals = useMemo(
    () => ({
      ...
      activeOrganizations: snapshot.organizations.filter(
      ...
      >
        <div>
          <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-electric-500/20 bg-electric-500/10 px-3 py-1 text-xs font-medium text-electric-300">
            <ShieldCheck className="h-3.5 w-3.5" /> Owner Runtime
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-white">
            Live Ownership Data
          </h1>
          <p className="mt-2 text-sm text-white/45">
            Authenticated runtime view for projects, organizations and users
            across the platform.
          </p>
        </div>
        <button
