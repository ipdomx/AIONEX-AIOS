...
export default function OwnerLicensingPage() {
  ...
  async function load(signal?: AbortSignal) {
    ...
    try {
      ...
      if (!(error instanceof DOMException && error.name === "AbortError")) setMessage("License synchronization failed.");
    } finally {
  ...
  async function act(id: string, action: "renew" | "suspend" | "restore") {
    ...
    try {
      await updateOwnerLicense(id, action);
      setItems((current) => current.map((item) => item.id === id ? { ...item, status: action === "suspend" ? "suspended" : "active" } : item));
      setMessage(`License action completed: ${action}.`);
    } catch {
      setMessage("Backend endpoint unavailable; license action was not persisted.");
    }
  }
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <div>
          <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-electric-500/20 bg-electric-500/10 px-3 py-1 text-xs font-medium text-electric-300"><KeyRound className="h-3.5 w-3.5" /> Owner Licensing</div>
          <h1 className="text-3xl font-bold tracking-tight text-white">Enterprise Licensing & Entitlements</h1>
          <p className="mt-2 text-sm text-white/45">Owner control for plans, seats, renewals, suspensions, restoration and commercial visibility.</p>
        </div>
        ...
        {items.map((item, index) => (
          <motion.div key={item.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.03 }} className="glass-card p-5">
            <div className="flex items-start justify-between gap-4">
              <div><h2 className="text-sm font-semibold text-white">{item.organization}</h2><p className="mt-1 text-xs text-white/40">{item.plan} plan · expires {item.expiresAt}</p></div>
              <span className={`rounded-full border px-2.5 py-1 text-xs ${statusClass[item.status]}`}>{item.status}</span>
            </div>
            <div className="mt-4 grid grid-cols-3 gap-3">
              <div className="rounded-lg bg-white/[0.02] p-3"><div className="text-[10px] uppercase tracking-wider text-white/30">Seats</div><div className="mt-1 text-sm font-semibold text-white">{item.seats}</div></div>
              <div className="rounded-lg bg-white/[0.02] p-3"><div className="text-[10px] uppercase tracking-wider text-white/30">Active</div><div className="mt-1 text-sm font-semibold text-white">{item.activeSeats}</div></div>
              <div className="rounded-lg bg-white/[0.02] p-3"><div className="text-[10px] uppercase tracking-wider text-white/30">Monthly</div><div className="mt-1 text-sm font-semibold text-white">€{item.monthlyValue.toLocaleString()}</div></div>
            </div>
            <div className="mt-4 flex flex-wrap gap-2">
              <button onClick={() => void act(item.id, "renew")} className="rounded-lg border border-green-500/20 bg-green-500/10 px-3 py-2 text-xs text-green-300"><RotateCcw className="mr-1 inline h-3.5 w-3.5" />Renew</button>
              {item.status === "suspended" ? (
                <button onClick={() => void act(item.id, "restore")} className="rounded-lg border border-blue-500/20 bg-blue-500/10 px-3 py-2 text-xs text-blue-300"><BadgeCheck className="mr-1 inline h-3.5 w-3.5" />Restore</button>
              ) : (
                <button onClick={() => void act(item.id, "suspend")} className="rounded-lg border border-red-500/20 bg-red-500/10 px-3 py-2 text-xs text-red-300"><ShieldAlert className="mr-1 inline h-3.5 w-3.5" />Suspend</button>
              )}
            </div>
          </motion.div>
        ))}
