...
export default function OwnerLicensingPage() {
  ...
  async function load(signal?: AbortSignal) {
    ...
    try {
      ...
      if (!(error instanceof DOMException && error.name === "AbortError"))
        setMessage("License synchronization failed.");
    } finally {
  ...
  async function act(id: string, action: "suspend" | "restore") {
    ...
    try {
      ...
      setMessage(
        error instanceof Error ? error.message : "License action failed.",
      );
    }
  }
      ...
      >
        <div>
          ...
          <p className="mt-2 text-sm text-white/45">
            License visibility for plans, seats and expiry readiness, with
            suspension and restoration controls backed by the current
        ...
        {items.map((item, index) => (
          ...
          >
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              <span
                className="rounded-lg border border-white/[0.08] bg-white/[0.03] px-3 py-2 text-xs text-white/35"
                title="Renewal requires an entitlement expiry record, which is not configured in the current registry."
              >
                Renewal unavailable
              </span>
            </div>
