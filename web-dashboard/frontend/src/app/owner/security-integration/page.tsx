...
export default function OwnerSecurityIntegrationPage() {
  ...
  const [message, setMessage] = useState(
    "Synchronizing security and governance integrations...",
  );
  ...
  const summary = useMemo(
    () => ({
      secure: snapshot.targets.filter((item) => item.status === "secure")
        .length,
      warning: snapshot.targets.filter((item) => item.status === "warning")
        .length,
      critical: snapshot.targets.filter((item) => item.status === "critical")
        .length,
    }),
    [snapshot.targets],
  );
      ...
      >
        <div>
          <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-electric-500/20 bg-electric-500/10 px-3 py-1 text-xs font-medium text-electric-300">
            <ShieldAlert className="h-3.5 w-3.5" /> Owner Security Integration
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-white">
            Security, Identity & Governance
          </h1>
          <p className="mt-2 text-sm text-white/45">
            Live owner visibility for IAM, external secret references, critical
        ...
        {snapshot.targets.map((item, index) => (
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-xs text-white/40">
                  {item.category} · Score {item.score}% · {item.last_checked_at}
                </p>
              </div>
              <span
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              {item.id === "threat-defense" && item.status === "critical" && (
                <button
                  disabled={loading || actingTarget !== null}
                  onClick={() => void command(item.id, "acknowledge")}
