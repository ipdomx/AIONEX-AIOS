...
export default function OwnerPlatformIntegrationPage() {
  ...
  const [message, setMessage] = useState(
    "Synchronizing platform integrations...",
  );
  ...
  const summary = useMemo(
    () => ({
      connected: snapshot.targets.filter((item) => item.status === "connected")
        .length,
      degraded: snapshot.targets.filter((item) => item.status === "degraded")
        .length,
      disconnected: snapshot.targets.filter(
  ...
  async function command(targetId: string, action: "validate") {
      ...
      >
        <div>
          <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-electric-500/20 bg-electric-500/10 px-3 py-1 text-xs font-medium text-electric-300">
            <Cable className="h-3.5 w-3.5" /> Owner Platform Integration
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-white">
            Platform Connectivity
          </h1>
          <p className="mt-2 text-sm text-white/45">
            Live core-data probes plus configuration status for AI providers and
            cloud credentials.
          </p>
        </div>
        <button
        ...
        {snapshot.targets.map((item, index) => (
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-xs text-white/40">
                  {item.category} · Health {item.health}%
                </p>
              </div>
              <span
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              >
                {item.configured ? "Open registry" : "Configure integration"}
              </Link>
            </div>
          </motion.div>
        ))}
