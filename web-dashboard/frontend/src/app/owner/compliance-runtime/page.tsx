...
export default function OwnerComplianceRuntimePage() {
  ...
  async function load(signal?: AbortSignal) {
    ...
    try {
      ...
      if (!(error instanceof DOMException && error.name === "AbortError")) setMessage("Compliance synchronization failed.");
    } finally {
  ...
  async function attest(id: string) {
    ...
    try {
      await attestComplianceControl(id);
      setItems((current) => current.map((item) => item.id === id ? { ...item, status: "compliant", updatedAt: "Just now" } : item));
      setMessage("Owner attestation completed.");
    } catch {
      setMessage("Backend endpoint unavailable; attestation was not persisted.");
    }
  }
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <div>
          <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-electric-500/20 bg-electric-500/10 px-3 py-1 text-xs font-medium text-electric-300"><FileCheck2 className="h-3.5 w-3.5" /> Owner Compliance Runtime</div>
          <h1 className="text-3xl font-bold tracking-tight text-white">Compliance Controls & Evidence</h1>
          <p className="mt-2 text-sm text-white/45">Owner-level control assurance for ISO 27001, SOC 2, GDPR and internal governance frameworks.</p>
        </div>
        ...
        {items.map((item, index) => (
          <motion.div key={item.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.03 }} className="glass-card p-5">
            <div className="flex items-start justify-between gap-4">
              <div><h2 className="text-sm font-semibold text-white">{item.framework} · {item.control}</h2><p className="mt-1 text-xs text-white/40">Owner: {item.owner} · Evidence: {item.evidence} · {item.updatedAt}</p></div>
              <span className={`rounded-full border px-2.5 py-1 text-xs ${statusClass[item.status]}`}>{item.status}</span>
            </div>
            <button onClick={() => void attest(item.id)} className="mt-4 rounded-lg border border-green-500/20 bg-green-500/10 px-3 py-2 text-xs text-green-300"><BadgeCheck className="mr-1 inline h-3.5 w-3.5" />Attest control</button>
          </motion.div>
        ))}
      </div>
    </div>
