...
import { CheckCircle2, Globe2, RefreshCw, ShieldCheck, TriangleAlert, XCircle } from "lucide-react";
import {
...
export default function OwnerProductionRuntimePage() {
  ...
  async function command(targetId: string, action: ProductionRuntimeAction) {
    ...
    try {
      ...
      setSnapshot(data);
      setMessage(`Production runtime ${action} completed.`);
    } catch {
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <div>
          <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-electric-500/20 bg-electric-500/10 px-3 py-1 text-xs font-medium text-electric-300"><Globe2 className="h-3.5 w-3.5" /> Production Runtime</div>
          <h1 className="text-3xl font-bold tracking-tight text-white">Deployment-Ready Runtime</h1>
          <p className="mt-2 text-sm text-white/45">Final runtime preparation for ai.vip-e.net and environment-driven production deployment.</p>
        </div>
      ...
      <div className="glass-card p-5">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div><div className="text-xs uppercase tracking-wider text-white/30">Production completion</div><div className="mt-2 text-4xl font-bold text-white">{snapshot.completion}%</div></div>
          <div className="text-xs text-white/50"><div>{snapshot.public_origin}</div><div className="mt-1">{snapshot.api_origin}</div></div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        {snapshot.targets.map((item, index) => (
          <motion.div key={item.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.03 }} className="glass-card p-5">
            ...
            <div className="mt-4 flex flex-wrap gap-2"><button onClick={() => void command(item.id, "validate")} className="rounded-lg border border-white/[0.08] bg-white/[0.04] px-3 py-2 text-xs text-white/70">Validate</button><button onClick={() => void command(item.id, "prepare")} className="rounded-lg border border-electric-500/20 bg-electric-500/10 px-3 py-2 text-xs text-electric-300">Prepare</button></div>
          </motion.div>
        ))}
      </div>
    </div>
