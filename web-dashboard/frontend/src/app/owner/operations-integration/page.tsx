...
import { Activity, CheckCircle2, RefreshCw, RotateCcw, ShieldCheck, TriangleAlert, XCircle } from "lucide-react";
import {
...
const emptySnapshot: OperationsSnapshot = { generated_at: "", completion: 0, targets: [] };
...
export default function OwnerOperationsIntegrationPage() {
  ...
  const [message, setMessage] = useState("Synchronizing enterprise operations...");
  ...
  const summary = useMemo(() => ({
    healthy: snapshot.targets.filter((item) => item.status === "healthy").length,
  ...
  async function command(targetId: string, action: OperationsAction) {
    ...
    try {
      ...
      setSnapshot(data);
      setMessage(`Operations ${action} completed.`);
    } catch {
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <div>
          <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-electric-500/20 bg-electric-500/10 px-3 py-1 text-xs font-medium text-electric-300"><Activity className="h-3.5 w-3.5" /> Owner Operations Integration</div>
          <h1 className="text-3xl font-bold tracking-tight text-white">Enterprise Operations Connectivity</h1>
          <p className="mt-2 text-sm text-white/45">Live owner integration for monitoring, logging, alerting, backup and disaster recovery.</p>
        </div>
      ...
      <div className="glass-card p-5">
        <div className="flex items-center justify-between gap-4"><div><div className="text-xs uppercase tracking-wider text-white/30">Operations readiness</div><div className="mt-2 text-4xl font-bold text-white">{snapshot.completion}%</div></div><ShieldCheck className="h-8 w-8 text-electric-300" /></div>
        ...
        {snapshot.targets.map((item, index) => (
          <motion.div key={item.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.03 }} className="glass-card p-5">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              <button onClick={() => void command(item.id, "recover")} className="rounded-lg border border-electric-500/20 bg-electric-500/10 px-3 py-2 text-xs text-electric-300"><RotateCcw className="mr-1 inline h-3.5 w-3.5" />Recover</button>
            </div>
          </motion.div>
        ))}
      </div>
