...
const inputClass =
  "glass-input w-full rounded-xl px-4 py-3 text-sm text-white outline-hidden disabled:cursor-not-allowed disabled:opacity-50";
...
const kinds: Array<{
...
}> = [
  {
    id: "image",
    title: "Image",
    description: "GPT Image 2 governed generation and edits",
    icon: ImageIcon,
    capability: "image",
  },
  {
    id: "video",
    title: "Video",
    description: "Sora 2 durable multi-scene generation",
    icon: Film,
    capability: "video",
  },
  {
    id: "speech",
    title: "Speech",
    description: "Pinned stock-voice narration",
    icon: Mic2,
    capability: "speech",
  },
  {
    id: "transcript",
    title: "Transcript",
    description: "Governed STT / diarization from a media node",
    icon: Languages,
  ...
  },
  {
    id: "dubbing",
    title: "Dubbing",
    description: "Translated stock-voice dubbing from transcript evidence",
    icon: AudioLines,
    capability: "dubbing",
  },
  {
    id: "music",
    title: "Music",
    description: "Replicate Lyria or Stability fixed-price generation",
    icon: Music2,
    capability: "music",
  },
  {
    id: "song",
    title: "Full Song",
    description: "RunPod ACE-Step full song + four stems",
    icon: Sparkles,
    capability: "open_song",
  },
];
...
function amount(value: string) {
  ...
  if (!Number.isFinite(parsed) || parsed <= 0)
    throw new Error("Enter a valid positive cost approval.");
  return parsed;
}

function statusTone(status: string) {
  if (status === "completed") return "text-emerald-300";
  if (["failed", "cancelled", "blocked", "needs_review"].includes(status))
    return "text-rose-300";
  if (["queued", "running", "rendering", "submitted"].includes(status))
...
export default function LiveMediaStudioPage() {
  ...
  const [
    syntheticVoiceDisclosureAccepted,
    setSyntheticVoiceDisclosureAccepted,
  ] = useState(false);
  ...
  const [songConcept, setSongConcept] = useState(
    "Original cinematic electronic pop with a memorable chorus and a clean resolved ending.",
  );
  ...
  const selected = useMemo(
    () => kinds.find((item) => item.id === kind) ?? kinds[0],
    [kind],
  );
  ...
  const load = useCallback(async () => {
    try {
      ...
      setMessage(
        "Live provider evidence, worker state, projects, and durable jobs synchronized.",
      );
    } catch (error) {
      setMessage(
        error instanceof Error ? error.message : "Live Media is unavailable.",
      );
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    if (kind === "speech" || kind === "dubbing")
      setSyntheticVoiceDisclosureAccepted(false);
    if (kind === "video") setCost("2.40");
    else if (kind === "song") setCost("0.20");
    else if (kind === "music")
      setCost(musicProvider === "stability" ? "0.20" : "0.04");
  ...
  async function submit(event: FormEvent) {
    ...
    if (!currentCapability?.ready) {
      setMessage(
        "This runtime is not currently live-ready for the selected provider/model evidence.",
      );
      return;
    }
    if (
      ...
      setMessage(
        "You must acknowledge the synthetic stock-voice disclosure before this request can be queued.",
      );
      return;
    }
    setBusy(true);
    try {
      ...
      if (kind === "image") {
        ...
        payload = {
          ...
          style,
          reference_node_id:
            videoOperation === "text-to-video" ? null : referenceNode || null,
        ...
        payload = {
          ...
          rights_basis:
            rightsBasis === "instrumental"
              ? "original"
              : rightsBasis.replace("original-user-owned", "original"),
          rights_evidence_sha256: rightsEvidence || null,
          commercial_use_authorized: commercialRights,
          provider_terms_accepted: providerTerms,
          ai_generated_disclosure_accepted: disclosureAccepted,
      ...
      setMessage(
        `${selected.title} request admitted safely. Durable status: ${String(result.status ?? "accepted")}.`,
      );
      await load();
    } catch (error) {
      setMessage(
        error instanceof Error ? error.message : "Live media request failed.",
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-bold text-white">
            Governed Live Media Studio
          </h1>
          <p className="mt-2 max-w-4xl text-sm leading-6 text-white/50">
            Real provider execution through the accepted Phase 36 authorities.
            Every paid request is tenant-scoped, idempotent, explicitly
            cost-approved, fail-closed, and never exposes provider credentials
            or raw job IDs.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Link href="/studio" className="btn-secondary">
            Provider-neutral Studio
          </Link>
          <button
            type="button"
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between gap-2">
                ...
                />
                <span
                  className={`text-[10px] font-semibold uppercase ${state?.ready ? "text-emerald-300" : "text-rose-300"}`}
                >
      ...
      <form
        onSubmit={submit}
        className="glass-card grid gap-6 p-6 xl:grid-cols-[1fr_360px]"
      >
        <div className="space-y-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-semibold text-white">
                {selected.title} request
              </h2>
              <p className="text-xs text-white/40">
                Worker:{" "}
                {currentCapability?.worker_live
                  ? "live"
                  : "disabled until deployment activation"}
              </p>
            </div>
          </div>
            ...
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <label className={labelClass}>
                ...
                <select
                  className={inputClass}
                  value={imageUseCase}
                  onChange={(e) => setImageUseCase(e.target.value)}
                >
              ...
              <label className={labelClass}>
                ...
                >
                  ...
                  <option value="social-portrait">Social portrait</option>
                  <option value="story-vertical">Story vertical</option>
                  <option value="ad-landscape">Ad landscape</option>
                  <option value="poster-portrait">Poster portrait</option>
                  <option value="logo-square">Logo square</option>
                </select>
              </label>
              {imageOperation !== "generate" && (
                <label className={labelClass}>
                  Reference media node
                  <input
                    className={inputClass}
                    value={referenceNode}
              ...
              {imageOperation === "inpaint" && (
                <label className={labelClass}>
                  ...
                  <input
                    className={inputClass}
                    value={maskNode}
                    onChange={(e) => setMaskNode(e.target.value)}
                  />
                </label>
              )}
              <label className={labelClass}>
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <label className={labelClass}>
                ...
                <select
                  ...
                  onChange={(e) => setVideoOperation(e.target.value)}
                >
              ...
              <label className={labelClass}>
                ...
                <select
                  className={inputClass}
                  value={videoUseCase}
                  onChange={(e) => setVideoUseCase(e.target.value)}
                >
                  <option value="advertisement">Advertisement</option>
          ...
          {(kind === "speech" || kind === "dubbing") && (
            <div className="rounded-xl border border-amber-400/20 bg-amber-400/5 p-4 text-xs leading-5 text-amber-100/80">
              <div className="font-semibold text-amber-100">
                Synthetic stock-voice disclosure
              </div>
              <p className="mt-1">
                This output uses an AI-generated synthetic stock voice. It must
                not be presented as a real person&apos;s voice or as an identity
                clone.
              </p>
              <label className="mt-3 flex items-start gap-3 text-white/70">
                <input
                  type="checkbox"
                  className="mt-0.5"
                  checked={syntheticVoiceDisclosureAccepted}
                  onChange={(e) =>
                    setSyntheticVoiceDisclosureAccepted(e.target.checked)
                  }
                />
                I understand and accept the synthetic-voice disclosure for this
                generated output.
              </label>
            </div>
          )}
          ...
          {kind === "transcript" && (
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <label className={labelClass}>
                ...
                <select
                  className={inputClass}
                  value={transcriptOperation}
                  onChange={(e) => setTranscriptOperation(e.target.value)}
                >
                  <option value="transcribe">Single-speaker transcript</option>
          ...
          {kind === "dubbing" && (
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <label className={`${labelClass} md:col-span-2`}>
                Speaker → stock voice JSON
                <textarea
                  className={`${inputClass} min-h-24 font-mono text-xs`}
                  value={voiceBindings}
                  onChange={(e) => setVoiceBindings(e.target.value)}
                />
              </label>
              <label className={labelClass}>
                Translation cap USD
                <input
                  ...
                  type="number"
                  step="0.01"
                  value={translationCap}
                  onChange={(e) => setTranslationCap(e.target.value)}
                />
              </label>
              <label className={labelClass}>
                Per-segment speech cap USD
                <input
                  className={inputClass}
                  type="number"
                  step="0.01"
                  value={segmentCap}
                  onChange={(e) => setSegmentCap(e.target.value)}
                />
              </label>
            </div>
          )}
          ...
          {kind === "music" && (
            <div className="grid gap-4 md:grid-cols-2">
              <label className={labelClass}>
                ...
                >
                  <option value="replicate">Replicate / Lyria</option>
                  <option value="stability">
                    Stability / Stable Audio 2.5
                  </option>
                </select>
              </label>
              <label className={labelClass}>
                ...
                <select
                  className={inputClass}
                  value={musicTier}
                  onChange={(e) => setMusicTier(e.target.value)}
                >
              ...
              <label className={labelClass}>
                ...
                >
                  <option value="instrumental">
                    Instrumental / no lyric rights claim
                  </option>
                  <option value="original-user-owned">
                    Original user-owned
                  </option>
              ...
              {musicTier === "final" && (
                <>
                  ...
                  <label className={labelClass}>
                    Final approval evidence SHA-256
                    <input
                      className={inputClass}
                      value={finalApprovalEvidence}
                      onChange={(e) => setFinalApprovalEvidence(e.target.value)}
                    />
                  </label>
                </>
              )}
              {!rightsBasis.startsWith("instrumental") && (
                <label className={`${labelClass} md:col-span-2`}>
                  Lyrics
                  <textarea
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <label className={labelClass}>
                ...
                <select
                  ...
                  onChange={(e) => setRightsBasis(e.target.value)}
                >
                  <option value="original-user-owned">Original</option>
                  <option value="licensed">Licensed</option>
                  <option value="public-domain">Public domain</option>
                </select>
              </label>
              {rightsBasis !== "instrumental" &&
                rightsBasis !== "original-user-owned" && (
                  <label className={`${labelClass} md:col-span-2`}>
                    Rights evidence SHA-256
                    <input
                      className={inputClass}
          ...
          {needsRights && (
            <div className="grid gap-3 rounded-xl border border-white/10 bg-white/2.5 p-4 text-xs text-white/60">
              <label className="flex items-start gap-3">
                ...
                />
                I confirm I hold the required commercial-use rights for the
                submitted material.
              </label>
              <label className="flex items-start gap-3">
                <input
                ...
                />
                I accept the selected provider&apos;s applicable terms.
              </label>
              <label className="flex items-start gap-3">
                <input
                  type="checkbox"
                ...
                />
                I accept required AI-generated-content disclosure obligations.
              </label>
            </div>
          )}
        </div>
        ...
        <aside className="space-y-4">
          <div className="rounded-xl border border-white/10 bg-white/2.5 p-4">
            ...
            <div className="mt-4 space-y-1 text-[11px] leading-5 text-white/40">
              ...
              <div>
                • Cost approval is durable before a job becomes claimable.
              </div>
              <div>
                • Open Song balance is checked only inside its secret-bearing
                worker.
              </div>
              <div>
                • Provider credentials and raw remote job IDs never return to
                this UI.
              </div>
            </div>
          </div>
          <button
            type="submit"
            disabled={busy || !currentCapability?.ready}
      ...
      <div className="glass-card p-5">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h2 className="font-semibold text-white">
              Recent live-media executions
            </h2>
            <p className="mt-1 text-xs text-white/40">
              Durable tenant-scoped status only; provider secrets and raw remote
              IDs are excluded.
            </p>
          </div>
          {activeJobs && (
          ...
          <table className="w-full min-w-[760px] text-left text-xs">
            <thead className="text-white/35">
              <tr>
                ...
                <th className="pb-3">Actual cost</th>
            ...
            <tbody className="divide-y divide-white/5">
              {jobs.slice(0, 40).map((job) => (
                <tr key={`${job.kind}-${job.id}`}>
                  <td className="py-3 font-medium capitalize text-white">
                    {job.kind}
                    {job.scene_key ? ` · ${job.scene_key}` : ""}
                  </td>
                  ...
                  <td
                    className={`py-3 font-semibold ${statusTone(job.status)}`}
                  >
                    {job.status}
                  </td>
                  <td className="py-3 text-white/50">
              ...
              {!jobs.length && (
                <tr>
                  <td colSpan={6} className="py-8 text-center text-white/35">
                    No live-media executions yet.
                  </td>
                </tr>
              )}
            </tbody>
