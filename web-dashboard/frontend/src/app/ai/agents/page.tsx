...
export default function AIAgentsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setMessage(
        error instanceof Error ? error.message : "AI runtime load failed",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function createAgent(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await runtimeServices.createAgent({
        ...
        system_prompt:
          String(form.get("system_prompt") || "").trim() || undefined,
      });
  ...
  async function execute(agent: AgentSummary) {
    ...
    try {
      ...
      setMessage(
        `Execution ${job.id} queued. Refresh to observe durable status.`,
      );
      await load();
    } catch (error) {
      setMessage(
  ...
  async function togglePause(agent: AgentSummary) {
    ...
    try {
      ...
      setMessage(
        error instanceof Error ? error.message : "Agent state update failed",
      );
    } finally {
      setBusy(null);
    }
  ...
  async function remove(agent: AgentSummary) {
    if (
      !window.confirm(
        `Delete ${agent.name}? This audited action cannot be undone.`,
      )
    )
      return;
    ...
    try {
      ...
      setMessage(
        error instanceof Error ? error.message : "Agent deletion failed",
      );
    } finally {
      setBusy(null);
    }
      ...
      >
        <div>
          ...
          <p className="mt-1 text-sm text-white/40">
            Durable provider-backed agents. No local placeholder state or
            synthetic execution success.
          </p>
        </div>
        <div className="flex gap-2">
          <button
            ...
            onClick={() => void load()}
            disabled={loading}
            className="glass rounded-xl px-3 py-2 text-sm text-white/70 disabled:opacity-50"
          >
            <RefreshCw className="me-2 inline h-4 w-4" />
            Refresh
          </button>
          <button
            type="button"
      ...
      {showCreate && (
        ...
        >
          ...
          >
            ...
            {providers
              .filter(
                (provider) => provider.configured !== false && provider.enabled,
              )
              .map((provider) => (
                <option
                  key={provider.id}
          ...
          <button
            disabled={
              busy === "create" ||
              !providers.some(
                (provider) => provider.enabled && provider.configured !== false,
              )
            }
      ...
      <div className="glass-card p-4">
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          >
            ...
            <option value="running" className="bg-space-800">
              Running
            </option>
            <option value="idle" className="bg-space-800">
              Idle
            </option>
            <option value="paused" className="bg-space-800">
              Paused
            </option>
            <option value="error" className="bg-space-800">
              Error
            </option>
          </select>
        </div>
            ...
            >
              <div className="mb-4 flex items-start justify-between">
                <div className="flex items-center gap-3">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-white/8 bg-linear-to-br from-purple-500/20 to-blue-500/20">
                ...
                <span
                  className={`rounded-full border px-2 py-0.5 text-[10px] font-medium ${statusClass(agent.status)}`}
                >
                  {agent.status}
                </span>
              </div>
              <div className="mb-4 grid grid-cols-2 gap-3">
                ...
                <div className="rounded-lg bg-white/2 p-2.5">
                  ...
                  <span className="text-sm font-bold text-white">
                    {agent.tasks_failed.toLocaleString()}
                  </span>
                </div>
                <div className="rounded-lg bg-white/2 p-2.5">
                  <div className="mb-1 flex items-center gap-1.5">
              ...
              <div className="flex gap-2 border-t border-white/6 pt-3">
                <button
                  ...
                  onClick={() => void execute(agent)}
                  className="glass flex flex-1 items-center justify-center gap-1.5 rounded-lg px-2 py-2 text-xs text-white/70 disabled:opacity-35"
                >
                ...
                <button
                  type="button"
                  disabled={Boolean(busy) || agent.status === "running"}
                  onClick={() => void togglePause(agent)}
                  className="glass flex items-center justify-center gap-1.5 rounded-lg px-3 py-2 text-xs text-white/70 disabled:opacity-35"
                >
                ...
                <button
                  ...
                  className="glass rounded-lg p-2 text-red-300 disabled:opacity-35"
