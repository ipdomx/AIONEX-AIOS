...
export default function Page() {
  ...
  return (
    <LiveDataPanel
      title="Logs"
      subtitle="Durable monitoring logs from the backend audit ledger."
      loading={l}
      error={e}
      empty={!rows.length}
    >
      ...
      <div className="space-y-3">
        {rows.map((x) => (
          <JsonCard key={x.id} title={`${x.level} · ${x.service}`} value={x} />
        ))}
      </div>
    </LiveDataPanel>
  );
